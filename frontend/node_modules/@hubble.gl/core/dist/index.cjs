...
function checkVersion() {
  const version = true ? "2.0.0-alpha.4" : globalThis.HUBBLE_VERSION || "untranspiled source";
...
var PNGSequenceEncoder = class extends FrameEncoder {
  ...
  constructor(settings) {
    ...
    this.mimeType = import_zip.ZipWriter.mimeTypes[0] || "";
    this.extension = `.${(_a = import_zip.ZipWriter.extensions) == null ? void 0 : _a[0]}`;
  }
  start() {
    this.filemap = {};
  }
...
var DeckLayerKeyframes = class extends keyframes_default {
  id;
  constructor({ id, timings, keyframes, easings }) {
...
var VideoCapture = class {
  ...
  stop({ onComplete = void 0, onSave = void 0, onStopped = void 0, abort = false }) {
    if (this.isRecording()) {
      ...
      if (!abort) {
        this._save(onSave).then(finish).catch((reason) => console.error("Error saving video:", reason.message, reason.stack));
      }
      finish();
    }
  }
