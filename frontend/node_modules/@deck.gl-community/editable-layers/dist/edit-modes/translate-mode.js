...
import { getEditModeCoordinateSystem } from "./coordinate-system.js";
export class TranslateMode extends GeoJsonEditMode {
    ...
    getTranslateAction(startDragPoint, currentPoint, editType, props) {
        ...
        if (viewportDesc && screenSpace) {
            ...
            const movedFeatures = this._geometryBeforeTranslate.features.map(feature => translateFromCenter(clone(feature), distanceMoved, direction, coordinateSystem));
            for (let i = 0; i < selectedIndexes.length; i++) {
                const selectedIndex = selectedIndexes[i];
