...
exports.getLayerBlendingParameters = getLayerBlendingParameters;
exports.patchDeckRendererForPostProcessing = patchDeckRendererForPostProcessing;
 ...
 */
function getLayerBlendingParameters(layerBlending) {
  ...
  if (blendFunc.length >= 2) {
    if (!_constants.GL_BLEND_FUNC_TO_WEBGPU[blendFunc[0]]) _window.console.warn("Unmapped blend function: ".concat(blendFunc[0], ", falling back to 'one'"));
    if (!_constants.GL_BLEND_FUNC_TO_WEBGPU[blendFunc[1]]) _window.console.warn("Unmapped blend function: ".concat(blendFunc[1], ", falling back to 'zero'"));
    params.blendColorSrcFactor = _constants.GL_BLEND_FUNC_TO_WEBGPU[blendFunc[0]] || _constants.BLEND_FACTOR.ONE;
    params.blendColorDstFactor = _constants.GL_BLEND_FUNC_TO_WEBGPU[blendFunc[1]] || _constants.BLEND_FACTOR.ZERO;
  }
  if (blendFunc.length >= 4) {
    if (!_constants.GL_BLEND_FUNC_TO_WEBGPU[blendFunc[2]]) _window.console.warn("Unmapped blend function: ".concat(blendFunc[2], ", falling back to 'one'"));
    if (!_constants.GL_BLEND_FUNC_TO_WEBGPU[blendFunc[3]]) _window.console.warn("Unmapped blend function: ".concat(blendFunc[3], ", falling back to 'zero'"));
    params.blendAlphaSrcFactor = _constants.GL_BLEND_FUNC_TO_WEBGPU[blendFunc[2]] || _constants.BLEND_FACTOR.ONE;
    params.blendAlphaDstFactor = _constants.GL_BLEND_FUNC_TO_WEBGPU[blendFunc[3]] || _constants.BLEND_FACTOR.ZERO;
  } else {
  ...
  if (Array.isArray(blendEquation)) {
    params.blendColorOperation = _constants.GL_BLEND_EQ_TO_WEBGPU[blendEquation[0]] || _constants.BLEND_OPERATION.ADD;
    params.blendAlphaOperation = _constants.GL_BLEND_EQ_TO_WEBGPU[blendEquation[1]] || _constants.BLEND_OPERATION.ADD;
  } else if (blendEquation) {
    params.blendColorOperation = _constants.GL_BLEND_EQ_TO_WEBGPU[blendEquation] || _constants.BLEND_OPERATION.ADD;
...
var _deckRendererPatched = false;
function patchDeckRendererForPostProcessing() {
  if (_deckRendererPatched) return;
  _deckRendererPatched = true;
  var proto = _core.DeckRenderer.prototype;
  var original = typeof proto._resizeRenderBuffers === 'function' ? proto._resizeRenderBuffers : null;
  proto._resizeRenderBuffers = function _resizeRenderBufferPatched() {
    var _this$device,
      _this = this;
    if (!((_this$device = this.device) !== null && _this$device !== void 0 && _this$device.canvasContext)) {
      return original === null || original === void 0 ? void 0 : original.call(this);
    }
    var renderBuffers = this.renderBuffers;
    var size = this.device.canvasContext.getDrawingBufferSize();
    var _size = (0, _slicedToArray2["default"])(size, 2),
      width = _size[0],
      height = _size[1];
    if (renderBuffers.length === 0) {
      [0, 1].map(function (i) {
        var colorTexture = _this.device.createTexture({
          sampler: {
            minFilter: _constants.FILTER_MODE.LINEAR,
            magFilter: _constants.FILTER_MODE.LINEAR
          },
          width: width,
          height: height
        });
        var depthTexture = _this.device.createTexture({
          id: "deck-renderbuffer-".concat(i, "-depth"),
          format: _constants.DEPTH_STENCIL_FORMAT.DEPTH24_PLUS,
          usage: _constants.TEXTURE_USAGE.RENDER_ATTACHMENT | _constants.TEXTURE_USAGE.SAMPLE,
          width: width,
          height: height
        });
        renderBuffers.push(_this.device.createFramebuffer({
          id: "deck-renderbuffer-".concat(i),
