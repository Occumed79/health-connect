import { Events, OrientationAxis, ViewportStatus, ViewportType, VOILUTFunctionType, } from '../../../enums/index.js';
import { ActorRenderMode } from '../../../types/index.js';
import { getRenderBackendForRenderMode, getRenderSurfaceForRenderMode, isImageRenderMode, isVolumeRenderMode, } from '../../helpers/renderBackendRegistry.js';
import { isImageActor } from '../../../utilities/actorCheck.js';
...
import getMinMax from '../../../utilities/getMinMax.js';
import renderingEngineCache from '../../renderingEngineCache.js';
import { getCameraVectors } from '../../helpers/getCameraVectors.js';
import GenericViewport from '../GenericViewport.js';
import { getGenericViewportImageDisplaySet, isGenericViewportImageDisplaySet, } from '../genericViewportDisplaySetAccess.js';
import { DefaultPlanarDataProvider } from './DefaultPlanarDataProvider.js';
import { createPlanarRenderPathResolver } from './PlanarRenderPathResolver.js';
import { defaultPlanarRenderPathDecisionService } from './PlanarRenderPathDecisionService.js';
import { normalizePlanarOrientation, selectPlanarRenderPath, } from './planarRenderPathSelector.js';
import { clonePlanarOrientation } from './planarLegacyCompatibility.js';
import { normalizePlanarRotation } from './planarViewPresentation.js';
import { cloneDisplayArea, createDefaultPlanarViewState, normalizePlanarViewState, } from './planarViewState.js';
import { normalizePlanarScale, } from './planarCameraScale.js';
import { getPlanarProjectionPan, getPlanarProjectionScale, getPlanarProjectionSnapshot, getPlanarProjectionZoom, } from './planarProjectionAdapter.js';
import { getPlanarViewStateCanvasDimensions, resolvePlanarViewportView, } from './PlanarResolvedView.js';
import { createPlanarCpuVolumeSliceBasis, createPlanarVolumeSliceBasis, getVolumeImageIdIndexWorldPoint, } from './planarSliceBasis.js';
import PlanarMountedData from './PlanarMountedData.js';
import PlanarViewReferenceController from './PlanarViewReferenceController.js';
class PlanarViewport extends GenericViewport {
    ...
    constructor(args) {
        ...
        this.renderPipelineSwapId = 0;
        this.lastRenderPathErrorByDataId = new Map();
        this.mountOptionsByDataId = new Map();
    ...
    async addImages(stackInputs) {
        ...
        const supportsImageOverlays = isImageRenderMode(renderMode) ||
    ...
    renderBindings() {
        ...
        const renderBinding = (binding, dataId) => {
            if (!binding.render) {
                return;
            }
    ...
    async applyRenderingPipelineUpdate() {
        if (this.isDestroyed) {
            return;
        }
        const swapId = ++this.renderPipelineSwapId;
        const isStale = () => swapId !== this.renderPipelineSwapId || this.isDestroyed;
        let changed = false;
        const sourceBinding = this.getCurrentBinding();
    ...
    reportRenderPathError(error, dataId) {
    ...
    getInheritedOverlayRenderBackend(role) {
        if (role === 'source') {
            return undefined;
        }
        return getRenderBackendForRenderMode(this.getCurrentPlanarRendering()?.renderMode);
    }
    ...
    createInitialVolumeSliceState(planarData, renderMode, orientation) {
        ...
        const createSliceBasis = getRenderSurfaceForRenderMode(renderMode) === 'cpu'
        ...
        if (typeof initialImageIdIndex === 'number') {
            const sliceWorldPoint = getVolumeImageIdIndexWorldPoint(planarData.imageVolume, initialImageIdIndex);
            if (sliceWorldPoint) {
                return {
                    kind: 'volumePoint',
                    sliceWorldPoint,
