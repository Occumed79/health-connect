import type { ICamera, Point2, Point3 } from '../../../types/index.js';
import ResolvedViewportView from '../ResolvedViewportView.js';
import type { WSIImageDataMetadata, WSIMapViewLike } from '../../../utilities/WSIUtilities.js';
import type { WSIViewState } from './WSIViewportTypes.js';
type WSIResolvedViewState = {
    viewState: WSIViewState;
    canvasHeight: number;
    canvasWidth: number;
    frameOfReferenceUID?: string | null;
    metadata?: WSIImageDataMetadata;
    view: WSIMapViewLike;
};
declare class WSIResolvedView extends ResolvedViewportView<WSIResolvedViewState> {
    canvasToWorld(canvasPos: Point2): Point3;
    worldToCanvas(worldPos: Point3): Point2;
    getFrameOfReferenceUID(): string | undefined;
    withZoom(zoom: number, canvasPoint?: Point2): WSIResolvedView;
    protected buildICamera(): ICamera;
    private canvasToIndex;
    private getResolution;
    private getResolutionForZoom;
    private getCenterIndexForCanvasPoint;
    private cloneWithViewState;
}
export type { WSIResolvedViewState };
export default WSIResolvedView;
