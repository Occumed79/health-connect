...
import * as planar from '../utilities/planar.js';
...
class Viewport {
    ...
    getUseCustomRenderingPipeline() {
        return this.constructor.useCustomRenderingPipeline;
    }
    ...
    setDataList(_entries) {
        throw new Error('Unsupported operation setDataList');
    }
    setDisplaySets(...entries) {
        this._displaySets = entries;
    }
    async mountDisplaySets(entries, load) {
        const [entry] = entries;
        if (!entry?.displaySetId) {
            throw new Error(`${this.constructor.name}: setDisplaySets requires a displaySetId`);
        }
        await load(entry);
...
export function getVolumeCenterIJK(dimensions, direction, viewPlaneNormal) {
    const ijkAxes = [
    ...
    return dimensions.map((d, i) => i === sliceAxis && isAxisAligned ? Math.floor(d / 2) : (d - 1) / 2);
}
