...
import extendedVtkCamera from './vtkClasses/extendedVtkCamera.js';
...
import { getImageDataMetadata as getImageDataMetadataUtil } from '../utilities/getImageDataMetadata.js';
import { coreLog } from '../utilities/logger.js';
...
import * as windowLevelUtil from '../utilities/windowLevel.js';
...
import imageRetrieveMetadataProvider from '../utilities/imageRetrieveMetadataProvider.js';
import imageIdToURI from '../utilities/imageIdToURI.js';
import getVOIRangeFromWindowLevel from '../utilities/getVOIRangeFromWindowLevel.js';
...
import { getImagePlaneModule } from '../utilities/buildMetadata.js';
import { createEmptyVTKImageData } from './helpers/planarImageRendering.js';
import { Events, InterpolationType, MetadataModules, RequestType, VOILUTFunctionType, ViewportStatus, } from '../enums/index.js';
...
import { adjustInitialViewUp } from '../utilities/adjustInitialViewUp.js';
...
class StackViewport extends Viewport {
    ...
    setVOICPU(voiRange, options = {}) {
        ...
        if (typeof voiRange === 'undefined') {
            ...
            const { lower, upper } = getVOIRangeFromWindowLevel(wwToUse, wcToUse, image.voiLUTFunction);
    ...
    async setDisplaySets(...entries) {
        await this.mountDisplaySets(entries, async (entry) => {
            ...
            if (!dataSet?.imageIds?.length) {
                throw new Error(`[StackViewport] No registered imageIds for display set ${entry.displaySetId}`);
            }
            const initialImageIdIndex = typeof dataSet.initialImageIdIndex === 'number'
            ...
            function successCallback(image, imageIdIndex, imageId) {
                ...
                if (pixelData instanceof Float32Array && scaledWithNonIntegers) {
                    ...
                    for (let i = 0; i < numPixels; i++) {
                        const rescaledPixel = Math.max(0, Math.min(65535, Math.floor((pixelData[i] - intercept) / slope)));
    ...
    getLoaderImageOptions(imageId) {
        const imageIdIndex = this.imageIds.indexOf(imageId);
        const { transferSyntaxUID } = metaData.get(MetadataModules.TRANSFER_SYNTAX, imageId) || {};
        const options = {
            useRGBA: false,
            transferSyntaxUID,
            priority: 5,
    ...
    _updateToDisplayImageCPU(image) {
        ...
        this.voiRange = getVOIRangeFromWindowLevel(windowWidth, windowCenter, voiLUTFunction);
    ...
    _getInitialVOIRange(image) {
        ...
        let voiRange = getVOIRangeFromWindowLevel(windowWidth, windowCenter, voiLUTFunction);
    ...
    _getVOIRangeForCurrentImage() {
        ...
        return getVOIRangeFromWindowLevel(windowWidth, windowCenter, voiLUTFunction);
    }
