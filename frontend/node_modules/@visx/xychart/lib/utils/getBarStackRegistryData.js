...
const getNumericValue = bar => ((0, _shape.getFirstItem)(bar) + (0, _shape.getSecondItem)(bar)) / 2;
...
function getBarStackRegistryData(stackedData, comprehensiveDomain, horizontal) {
  ...
  return stackedData.map((data, index) => {
    ...
    if (comprehensiveDomain.length > 0 && index === 0) {
      if (horizontal) {
        entry.xScale = scale => scale.domain((0, _d3Array.extent)(scale.domain().concat(comprehensiveDomain)));
      } else {
        entry.yScale = scale => scale.domain((0, _d3Array.extent)(scale.domain().concat(comprehensiveDomain)));
      }
    }
    return entry;
  }).filter(entry => entry);
