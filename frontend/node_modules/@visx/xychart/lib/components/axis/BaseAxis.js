...
function BaseAxis(_ref) {
  ...
  const axisStyles = (0, _react.useMemo)(() => orientation === 'left' || orientation === 'right' ? theme?.axisStyles?.y?.[orientation] : theme?.axisStyles?.x?.[orientation], [theme, orientation]);
  const maybeTickLabelProps = props.tickLabelProps;
  const tickLabelProps = (0, _react.useMemo)(() => maybeTickLabelProps || axisStyles // construct from props + theme if possible
  ? (value, index, values) => (
  ...
  return scale && hasRegisteredData ? /*#__PURE__*/(0, _jsxRuntime.jsx)(AxisComponent, {
    top: topOffset,
    left: leftOffset,
