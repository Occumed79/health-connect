...
import BrushHandle from "./BrushHandle.js";
import BrushCorner from "./BrushCorner.js";
import BrushSelection from "./BrushSelection.js";
import BrushOverlay from "./BrushOverlay.js";
import { debounce, getPageCoordinates } from "./utils.js";
import { jsx as _jsx, jsxs as _jsxs } from "react/jsx-runtime";
export default class BaseBrush extends Component {
  constructor(props) {
    super(props);
    const {
      initialBrushPosition
    } = props;
    const extent = initialBrushPosition ? this.getExtent(initialBrushPosition.start, initialBrushPosition.end) : {
      x0: -1,
      x1: -1,
      y0: -1,
      y1: -1
  ...
  handleWindowPointerMove = event => {
    ...
    if (brushingType === 'move') {
      this.updateBrush(prevBrush => {
        ...
        const validDx = offsetX > 0 ? Math.min(offsetX, prevBrush.bounds.x1 - x1) : Math.max(offsetX, prevBrush.bounds.x0 - x0);
        const validDy = offsetY > 0 ? Math.min(offsetY, prevBrush.bounds.y1 - y1) : Math.max(offsetY, prevBrush.bounds.y0 - y0);
        return {
          ...prevBrush,
          isBrushing: true,
          extent: {
  ...
  handleBrushingTypeChange = (type, brushPageOffset) => {
    this.updateBrush(prevBrush => {
      const next = {
        ...prevBrush,
        brushingType: type,
  ...
  render() {
    ...
    return /*#__PURE__*/_jsxs(Group, {
      ...
      top: top,
      left: left,
      children: [disableDraggingOverlay ? /*#__PURE__*/_jsx(BrushOverlay, {
        width: stageWidth,
        height: stageHeight,
        onClick: event => {
        ...
        children: _ref => {
          ...
          return /*#__PURE__*/_jsx(BrushOverlay, {
            width: stageWidth,
            height: stageHeight,
      ...
      }), start && end && /*#__PURE__*/_jsx(BrushSelection, {
      ...
      }), start && end && Object.keys(handles).filter(handleKey => resizeTriggerAreaSet.has(handleKey)).map(handleKey => {
        const handle = handles[handleKey];
        return handle && /*#__PURE__*/_jsx(BrushHandle, {
          type: handleKey,
          handle: handle,
          stageWidth: stageWidth,
          stageHeight: stageHeight,
      ...
      }), start && end && Object.keys(corners).filter(cornerKey => resizeTriggerAreaSet.has(cornerKey)).map(cornerKey => {
        const corner = corners[cornerKey];
        return corner && /*#__PURE__*/_jsx(BrushCorner, {
          type: cornerKey,
          brush: this.state,
          updateBrush: this.updateBrush,
          stageWidth: stageWidth,
