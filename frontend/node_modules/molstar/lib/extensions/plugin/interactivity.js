 ...
 */
export function applyStructureInteractivity(plugin, { expression, elements, action: action_, applyGranularity = false, filterStructure, focusOptions }) {
    var _a;
    ...
    for (const s of structures) {
        ...
        for (const action of actions) {
            if (action === 'select') {
                ...
                focused = true;
                if (actions.length === 1)
                    return; // if only focusing, focus the first matching structure and return immediately
            }
        }
    }
}
