...
import { Viewport, cameraProject } from '../../../mol-canvas3d/camera/util.js';
import { Grid, Volume } from '../../../mol-model/volume.js';
import { Tensor, Mat4, Vec3, Vec4 } from '../../../mol-math/linear-algebra.js';
import { CustomProperties } from '../../../mol-model/custom-property.js';
import { pointInPolygon2D as pointInPolygon } from '../../../mol-math/geometry/polygon.js';
const tmpVec3 = Vec3();
const tmpVec4 = Vec4();
const tmpG2C = Mat4();
function prepareMask(mask) {
    ...
    const normPolygon = mask.polygon.map(([px, py]) => [px / w, py / h]);
...
const tmpNorm = [0, 0];
function projectToNormInPlace(worldPos, prepared) {
...
export async function computeVolumeMask(volume, params, ctx) {
    ...
    for (let i = 0; i < nx; i++) {
        ...
        for (let j = 0; j < ny; j++) {
            for (let k = 0; k < nz; k++) {
                if (space.get(data, i, j, k) < absThreshold)
                    continue;
                ...
                if (invertPolygons ? !passes : passes)
                    selected[space.dataOffset(i, j, k)] = 1;
            }
        }
    }
    if (dilation <= 0)
...
export function dilate3D(src, nx, ny, nz, radius, space) {
    const dst = new Uint8Array(src.length);
    const r2 = radius * radius;
    for (let i = 0; i < nx; i++) {
        for (let j = 0; j < ny; j++) {
            for (let k = 0; k < nz; k++) {
                if (src[space.dataOffset(i, j, k)] === 0)
                    continue;
 ...
 */
export function computeStructureMask(volume, atomPositions, radiusAngstrom) {
 ...
 */
export function computeSoftEdge(maskData, nx, ny, nz, softEdgeWidth, space) {
    const n = nx * ny * nz;
    ...
    for (let i = 0; i < nx; i++) {
        for (let j = 0; j < ny; j++) {
            for (let k = 0; k < nz; k++) {
                ...
                let d = dist[idx];
                if (i > 0)
                    d = Math.min(d, dist[space.dataOffset(i - 1, j, k)] + 1);
                if (j > 0)
                    d = Math.min(d, dist[space.dataOffset(i, j - 1, k)] + 1);
                if (k > 0)
                    d = Math.min(d, dist[space.dataOffset(i, j, k - 1)] + 1);
                if (i > 0 && j > 0)
                    d = Math.min(d, dist[space.dataOffset(i - 1, j - 1, k)] + 1.414);
                if (i > 0 && k > 0)
                    d = Math.min(d, dist[space.dataOffset(i - 1, j, k - 1)] + 1.414);
                if (j > 0 && k > 0)
                    d = Math.min(d, dist[space.dataOffset(i, j - 1, k - 1)] + 1.414);
                if (i > 0 && j > 0 && k > 0)
                    d = Math.min(d, dist[space.dataOffset(i - 1, j - 1, k - 1)] + 1.732);
                dist[idx] = d;
            }
        }
    }
...
function computeSigma(data, mean) {
    let sumSq = 0;
    for (let i = 0; i < data.length; i++)
        sumSq += (data[i] - mean) ** 2;
    return Math.sqrt(sumSq / data.length);
}
export function buildMaskVolume(source, data) {
    const srcGrid = source.grid;
    const [nx, ny, nz] = srcGrid.cells.space.dimensions;
    const axisOrder = srcGrid.cells.space.axisOrderSlowToFast.slice();
    ...
    return {
        ...
        sourceData: { kind: 'custom', name: 'Volume Mask', data: null },
