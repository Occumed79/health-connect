...
import { applyViewSnapshot } from './behavior.js';
import { StateTransforms } from '../../mol-plugin-state/transforms.js';
import { KinemageShapePointsProvider, KinemageShapeLinesProvider, KinemageShapeMeshProvider, KinemageShapeSpheresProvider } from './behavior.js';
...
export class KinemageControls extends CollapsableControls {
    defaultState() {
        return {
            header: 'Kinemage',
            isCollapsed: false,
    ...
    async applyView(kinData, viewKey) {
        ...
        if (snap) {
            await applyViewSnapshot(this.plugin, snap);
        }
    }
    ...
    renderControls() {
        ...
        if (kins.length === 0)
            return _jsx("div", { style: { padding: '6px' }, children: "No Kinemage data" });
        const blocks = [];
        for (const { kinData, visControllerRef } of kins) {
            const title = kinData.pdbfile || nameFromString(kinData.caption) || 'Kinemage';
            const kinBlock = [];
            // Title
            kinBlock.push(_jsx("div", { style: { padding: '6px', fontWeight: 'bold', borderBottom: '1px solid rgba(255,255,255,0.1)' }, children: title }, 'title-' + title));
            // views
            ...
            if (viewEntries.length > 0) {
                for (const [viewKey, viewObj] of viewEntries) {
                    ...
                    kinBlock.push(_jsx("div", { style: { padding: '2px 6px' }, children: _jsx("button", { className: 'msp-btn msp-btn-block', onClick: () => this.applyView(kinData, viewKey), title: `Apply view: ${label}`, children: label }) }, 'view-' + title + '-' + viewKey));
                }
            }
            // animate
            if (kinData.groupsAnimate && kinData.groupsAnimate.length > 0) {
                kinBlock.push(_jsx("div", { style: { padding: '2px 6px' }, children: _jsx("button", { className: 'msp-btn msp-btn-block', onClick: () => this.triggerAnimateForKin(visControllerRef, kinData, 'animate'), title: 'Cycle through animation frames', children: "Animate" }) }, 'anim-' + title));
            }
            if (kinData.groupsAnimate2 && kinData.groupsAnimate2.length > 0) {
                kinBlock.push(_jsx("div", { style: { padding: '2px 6px' }, children: _jsx("button", { className: 'msp-btn msp-btn-block', onClick: () => this.triggerAnimateForKin(visControllerRef, kinData, '2animate'), title: 'Cycle through second animation frames', children: "Animate2" }) }, 'anim2-' + title));
            }
            // groups
            for (const [groupKey, groupInfo] of Object.entries(kinData.groupDict || {})) {
                if (!groupInfo.nobutton) {
                    ...
                    kinBlock.push(_jsx("div", { style: { padding: '2px 6px' }, children: _jsxs("label", { style: { display: 'flex', alignItems: 'center', cursor: 'pointer' }, children: [_jsx("input", { type: 'checkbox', checked: visible, onChange: () => this.toggleVisibility(visControllerRef, kinData, { type: 'group', key: groupKey }), style: { marginRight: '6px' } }), _jsx("span", { title: label, children: label })] }) }, 'group-' + title + '-' + groupKey));
                }
                ...
                if (!groupInfo.dominant) {
                    for (const [subgroupKey, subgroupInfo] of Object.entries(kinData.subgroupDict || {})) {
                        if (subgroupKey.startsWith(groupKey + ':')) {
                            ...
                            kinBlock.push(_jsx("div", { style: { padding: '2px 6px', paddingLeft: '24px' }, children: _jsxs("label", { style: { display: 'flex', alignItems: 'center', cursor: 'pointer' }, children: [_jsx("input", { type: 'checkbox', checked: visible, onChange: () => this.toggleVisibility(visControllerRef, kinData, { type: 'subgroup', key: subgroupKey }), style: { marginRight: '6px' } }), _jsx("span", { title: subgroupLabel, children: subgroupLabel })] }) }, 'subgroup-' + title + '-' + subgroupKey));
                        }
                    }
                }
            }
            ...
            for (const [subgroupKey, subgroupInfo] of Object.entries(kinData.subgroupDict || {})) {
                ...
                if (subgroupInfo.nobutton)
                    continue;
                const visible = this.isVisible(visControllerRef, { type: 'subgroup', key: subgroupKey });
                kinBlock.push(_jsx("div", { style: { padding: '2px 6px' }, children: _jsxs("label", { style: { display: 'flex', alignItems: 'center', cursor: 'pointer' }, children: [_jsx("input", { type: 'checkbox', checked: visible, onChange: () => this.toggleVisibility(visControllerRef, kinData, { type: 'subgroup', key: subgroupKey }), style: { marginRight: '6px' } }), _jsx("span", { title: subgroupKey, children: subgroupKey })] }) }, 'subgroup-' + title + '-' + subgroupKey));
            }
            // masters
            for (const [masterKey] of Object.entries(kinData.masterDict || {})) {
                ...
                kinBlock.push(_jsx("div", { style: { padding: '2px 6px' }, children: _jsxs("label", { style: { display: 'flex', alignItems: 'center', cursor: 'pointer' }, children: [_jsx("input", { type: 'checkbox', checked: visible, onChange: () => this.toggleVisibility(visControllerRef, kinData, { type: 'master', key: masterKey }), style: { marginRight: '6px' } }), _jsx("span", { title: masterKey, children: masterKey })] }) }, 'master-' + title + '-' + masterKey));
            }
            blocks.push(_jsx("div", { className: 'msp-control-group-wrapper', children: kinBlock }, 'kin-block-' + title));
        }
