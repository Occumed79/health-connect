...
import { isSingleLigandLoci, superposeLigandsByMccs, DefaultLigandMccsOptions } from '../../mol-model/structure/structure/util/superposition-ligand.js';
...
import { ArrowDownwardSvg, ArrowUpwardSvg, DeleteOutlinedSvg, HelpOutlineSvg, Icon, SuperposeAtomsSvg, SuperposeChainsSvg, SuperposeLigandsSvg, SuperpositionSvg, TuneSvg } from '../controls/icons.js';
...
function structureHasLigand(structure) {
    if (!structure)
        return false;
...
export class SuperpositionControls extends PurePluginUIComponent {
    constructor() {
        ...
        this.superposeMccs = async () => {
            ...
            const coordinateSystem = (_b = (_a = pivot === null || pivot === void 0 ? void 0 : pivot.transform) === null || _a === void 0 ? void 0 : _a.cell.obj) === null || _b === void 0 ? void 0 : _b.data.coordinateSystem;
            const mccsAlignTask = Task.create('Ligand Superposition', async (ctx) => {
                let aligned = 0;
                for (let i = 1; i < entries.length; i++) {
                    if (ctx.shouldUpdate)
                        await ctx.update(`Superposing ligand ${i} of ${entries.length - 1}...`);
                    ...
                    if (result.truncated)
                        this.plugin.log.warn(`Ligand superposition: MCCS search for [${labelB}] hit its time budget; the alignment may not be optimal.`);
                }
        ...
        this.toggleByMccs = () => this.setState({ action: this.state.action === 'byMccs' ? void 0 : 'byMccs' });
    ...
    componentDidMount() {
        ...
        this.subscribe(this.plugin.managers.structure.hierarchy.behaviors.selection, sel => {
            this.setState({
                canUseDb: sel.structures.every(s => { var _a; return !!((_a = s.cell.obj) === null || _a === void 0 ? void 0 : _a.data) && s.cell.obj.data.models.some(m => SIFTSMapping.Provider.isApplicable(m)); }),
                // need at least two ligand-bearing structures to superpose ligands
                canUseLigands: sel.structures.filter(s => { var _a; return structureHasLigand((_a = s.cell.obj) === null || _a === void 0 ? void 0 : _a.data); }).length >= 2
            });
        });
    }
    get selection() {
    ...
    ligandsLociEntry(e, idx) {
        return _jsxs("div", { children: [_jsx("div", { className: 'msp-control-group-header', children: _jsx("div", { className: 'msp-no-overflow', title: e.label, children: e.label }) }), _jsx("div", { className: 'msp-control-offset', children: e.ligands.map((h, i) => this.historyEntry(h, i)) })] }, idx);
    }
    get chainEntries() {
    ...
    superposeByDbMapping() {
        return _jsx(_Fragment, { children: _jsx(Button, { icon: SuperposeChainsSvg, title: 'Superpose structures using intersection of residues from SIFTS UNIPROT mapping.', onClick: this.superposeDb, disabled: this.state.isBusy, children: "UniProt" }) });
    }
    addByMccs() {
        const entries = this.ligandEntries;
        return _jsxs(_Fragment, { children: [entries.length > 0 && _jsx("div", { className: 'msp-control-offset', children: entries.map((e, i) => this.ligandsLociEntry(e, i)) }), entries.length < 2 && _jsx("div", { className: 'msp-control-offset msp-help-text', children: _jsxs("div", { className: 'msp-help-description', children: [_jsx(Icon, { svg: HelpOutlineSvg, inline: true }), "Add 1 selection", this.toggleHint(), " per structure (2+ structures total). Selections must be confined to a single ligand residue. Fast path matches atom names when compound name matches."] }) }), entries.length > 1 && _jsx(Button, { title: 'Superpose structures by selected ligands (atom-name fast path, then Maximum Common Connected Subgraph).', className: 'msp-btn-commit msp-btn-commit-on', onClick: this.superposeMccs, style: { marginTop: '1px' }, children: "Superpose" })] });
    }
    render() {
        return _jsxs(_Fragment, { children: [_jsxs("div", { className: 'msp-flex-row', children: [_jsx(ToggleButton, { icon: SuperposeChainsSvg, label: 'Chains', title: 'Superpose selected polymer chains, optionally guided by a sequence alignment (see Options). Select one chain (or residues within it) per structure; 2+ structures.', toggle: this.toggleByChains, isSelected: this.state.action === 'byChains', disabled: this.state.isBusy }), _jsx(ToggleButton, { icon: SuperposeChainsSvg, label: 'TM-align', title: 'Sequence-independent structural alignment of polymer chains (TM-align) \u2014 best when sequence identity is low. Select one chain per structure; 2+ structures.', toggle: this.toggleByTMAlign, isSelected: this.state.action === 'byTMAlign', disabled: this.state.isBusy }), this.state.canUseDb && this.superposeByDbMapping(), _jsx(ToggleButton, { icon: TuneSvg, label: '', title: 'Options for chain- and UniProt-based superposition (sequence-guided alignment, trace/CA-only).', toggle: this.toggleOptions, isSelected: this.state.action === 'options', disabled: this.state.isBusy, style: { flex: '0 0 40px', padding: 0 } })] }), _jsxs("div", { className: 'msp-flex-row', children: [this.state.canUseLigands && _jsx(ToggleButton, { icon: SuperposeLigandsSvg, label: 'Ligands', title: 'Superpose ligands by atom-name match (identical compound) or maximum common connected subgraph (MCCS) otherwise. Select one ligand residue per structure; 2+ structures.', toggle: this.toggleByMccs, isSelected: this.state.action === 'byMccs', disabled: this.state.isBusy }), _jsx(ToggleButton, { icon: SuperposeAtomsSvg, label: 'Atoms', title: 'Superpose by manually selected atoms, paired in selection order. Pick the same atoms, in the same order, in each structure (1+ per structure).', toggle: this.toggleByAtoms, isSelected: this.state.action === 'byAtoms', disabled: this.state.isBusy })] }), this.state.action === 'byChains' && this.addByChains(), this.state.action === 'byAtoms' && this.addByAtoms(), this.state.action === 'byTMAlign' && this.addByTMAlign(), this.state.action === 'byMccs' && this.state.canUseLigands && this.addByMccs(), this.state.action === 'options' && _jsx("div", { className: 'msp-control-offset', children: _jsx(ParameterControls, { params: StructureSuperpositionParams, values: this.state.options, onChangeValues: this.setOptions, isDisabled: this.state.isBusy }) })] });
    }
}
