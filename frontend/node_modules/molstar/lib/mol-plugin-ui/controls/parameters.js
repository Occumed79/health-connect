...
export class LineGraphControl extends React.PureComponent {
    ...
    render() {
        ...
        const presetItems = hasPresets && this.state.showPresets
            ...
            : null;
        return _jsxs(_Fragment, { children: [_jsx(ControlRow, { label: label, control: _jsxs("div", { style: { display: 'flex', textAlignLast: 'center' }, children: [_jsx("button", { onClick: this.toggleExpanded, disabled: this.props.isDisabled, style: { flex: '1 1 auto', overflow: 'hidden', textOverflow: 'ellipsis' }, children: `${this.state.message}` }), hasPresets && _jsx(IconButton, { svg: BookmarksOutlinedSvg, onClick: this.toggleShowPresets, toggleState: this.state.showPresets, title: 'Presets', disabled: this.props.isDisabled, flex: true })] }) }), presetItems && _jsx("div", { className: 'msp-control-offset', children: _jsx(ActionMenu, { items: presetItems, onSelect: this.onSelectPreset }) }), _jsx("div", { className: 'msp-control-offset', style: { display: this.state.isExpanded ? 'block' : 'none', marginTop: 1 }, children: _jsx(LineGraphComponent, { data: this.props.value, volume: (_b = (_a = this.props.param).getVolume) === null || _b === void 0 ? void 0 : _b.call(_a), yAxis: this.props.param.yAxis, onChange: this.onChange, onHover: this.onHover, onDrag: this.onDrag }) })] });
    }
}
...
export class ObjectListControl extends React.PureComponent {
    ...
    render() {
        ...
        const control = hasPresets
            ? _jsxs("div", { className: 'msp-flex-row', children: [_jsx("button", { onClick: this.toggleExpanded, disabled: this.props.isDisabled, children: value }), _jsx(IconButton, { svg: BookmarksOutlinedSvg, onClick: this.toggleShowPresets, toggleState: this.state.showPresets, title: 'Presets', disabled: this.props.isDisabled })] })
            : _jsx("button", { onClick: this.toggleExpanded, disabled: this.props.isDisabled, children: value });
        return _jsxs(_Fragment, { children: [_jsx(ControlRow, { label: label, control: control }), hasPresets && this.state.showPresets && _jsx(ActionMenu, { items: this.presetItems(this.props.param), onSelect: this.onSelectPreset }), this.state.isExpanded && _jsxs("div", { className: 'msp-control-offset', children: [this.props.value.map((v, i) => _jsx(ObjectListItem, { param: this.props.param, value: v, index: i, actions: this.actions, isDisabled: this.props.isDisabled }, i)), _jsx(ControlGroup, { header: 'New Item', children: _jsx(ObjectListEditor, { params: this.props.param.element, apply: this.add, value: this.props.param.ctor(), isDisabled: this.props.isDisabled }) })] })] });
    }
}
