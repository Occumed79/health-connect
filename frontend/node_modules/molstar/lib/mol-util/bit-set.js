...
export class BitSet {
    ...
    static full(nbits) {
        ...
        const rem = nbits & 31;
        if (rem !== 0)
            b.words[b.words.length - 1] = ~(0xffffffff << rem);
        return b;
    }
    clone() {
    ...
    and(o) { const w = this.words, x = o.words; for (let k = 0, n = w.length; k < n; k++)
        w[k] &= x[k]; }
    /** In-place set difference with `o` (same capacity). */
    andNot(o) { const w = this.words, x = o.words; for (let k = 0, n = w.length; k < n; k++)
        w[k] &= ~x[k]; }
    /** In-place union with `o` (same capacity). */
    or(o) { const w = this.words, x = o.words; for (let k = 0, n = w.length; k < n; k++)
        w[k] |= x[k]; }
    isEmpty() {
        const w = this.words;
