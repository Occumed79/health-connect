...
exports.histogramPercentile = histogramPercentile;
exports.histogramRobustStats = histogramRobustStats;
 ...
 */
function histogramPercentile(h, p, excludeBin) {
 ...
 */
function histogramRobustStats(h, opts) {
    const { counts, min, binWidth } = h;
    const n = counts.length;
    let excludeBin;
    if ((opts === null || opts === void 0 ? void 0 : opts.ignoreZero) && binWidth > 0) {
 ...
 */
function downsampleHistogram(h, binCount) {
    const srcN = h.counts.length;
    if (binCount <= 0 || binCount >= srcN)
        return h;
    const counts = new Int32Array(binCount);
