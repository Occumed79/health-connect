...
const instance_grid_1 = require("../../mol-math/geometry/instance-grid.js");
...
function createTransform(transformArray, instanceCount, invariantBoundingSphere, cellSize, batchSize, transformData) {
    ...
    if (transformData) {
        ...
        mol_util_1.ValueCell.update(transformData.extraTransform, extraTransform);
        mol_util_1.ValueCell.updateIfChanged(transformData.hasExtraTransform, false);
        ...
        transformData = {
            ...
            extraTransform: mol_util_1.ValueCell.create(new Float32Array(instanceCount * 16)),
            hasExtraTransform: mol_util_1.ValueCell.create(false),
...
function updateTransformData(transformData, invariantBoundingSphere, cellSize, batchSize) {
    ...
    const matrixIsIdentity = linear_algebra_1.Mat4.isIdentity(matrix);
    if (!hasExtraTransform && matrixIsIdentity) {
        ...
        for (let i = 0; i < instanceCount; i++) {
            const i16 = i * 16;
            linear_algebra_1.Mat4.mulOffset(aTransform, matrix, transform, i16, 0, i16);
        }
    }
    else if (hasExtraTransform && matrixIsIdentity) {
        for (let i = 0; i < instanceCount; i++) {
