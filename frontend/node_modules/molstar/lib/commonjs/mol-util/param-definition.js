...
const lists_1 = require("./color/lists.js");
...
(function (ParamDefinition) {
    ...
    function ColorList(defaultValue, info) {
        ...
        if (typeof defaultValue === 'string') {
            const colors = (0, lists_1.getColorListFromName)(defaultValue);
    ...
    function Url(url, info) {
        const defaultValue = typeof url === 'string' ? assets_1.Asset.Url(url) : assets_1.Asset.Url(url.url, { body: url.body });
    ...
    function LineGraph(defaultValue, info) {
        ...
        if (info === null || info === void 0 ? void 0 : info.yAxis)
            ret.yAxis = info.yAxis;
        if (info === null || info === void 0 ? void 0 : info.getPresets)
            ret.getPresets = info.getPresets;
        return ret;
    }
    ...
    function MappedStatic(defaultKey, map, info) {
        const options = (info === null || info === void 0 ? void 0 : info.options)
            ? info.options
            : Object.keys(map).map(k => [k, map[k].label || (0, string_1.stringToWords)(k)]);
    ...
    function ObjectList(element, getLabel, info) {
        const ret = setInfo({ type: 'object-list', element: element, getLabel, ctor: _defaultObjectListCtor, defaultValue: (info === null || info === void 0 ? void 0 : info.defaultValue) || [] }, info);
        if (info === null || info === void 0 ? void 0 : info.presets)
            ret.presets = info.presets;
        return ret;
    }
    ...
    function Ref(ref = '') {
        return { ref, getValue: unsetGetValue };
    }
    ParamDefinition.Ref = Ref;
    function ValueRef(getOptions, resolveRef, info) {
        return setInfo({ type: 'value-ref', defaultValue: Ref(info === null || info === void 0 ? void 0 : info.defaultRef), getOptions, resolveRef }, info);
    }
    ParamDefinition.ValueRef = ValueRef;
    function DataRef(info) {
        return setInfo({ type: 'data-ref', defaultValue: Ref(info === null || info === void 0 ? void 0 : info.defaultRef) }, info);
    }
    ParamDefinition.DataRef = DataRef;
    function asDataRef(ref) {
        return { ref, getValue: unsetGetValue };
    }
    ParamDefinition.asDataRef = asDataRef;
    ...
    function collectRefValue(p, value, out) {
        if (value === undefined || value === null)
            return out;
        if (p.type === 'value-ref' || p.type === 'data-ref') {
            const v = value;
            if (v && typeof v.ref === 'string' && v.ref) {
                (0, array_1.arraySetAdd)(out, v.ref);
            }
        }
        else if (p.type === 'group') {
    ...
    function collectRefsImpl(params, values, out) {
        for (const n of Object.keys(params)) {
            collectRefValue(params[n], values === null || values === void 0 ? void 0 : values[n], out);
        }
        return out;
    }
    /**
     ...
     * parameter values.
     */
    function collectRefs(params, values, out) {
        if (!params || !values)
            return out;
        return collectRefsImpl(params, values, out);
    }
    ParamDefinition.collectRefs = collectRefs;
    ...
    function isParamEqual(p, a, b) {
        ...
        if (p.type === 'group') {
            ...
            if (u.length !== v.length)
                return false;
            for (let i = 0, _i = u.length; i < _i; i++) {
                if (!vec2_1.Vec2.areEqual(u[i], v[i]))
                    return false;
            }
            return true;
        }
        else if (p.type === 'vec3') {
            return vec3_1.Vec3.equals(a, b);
        }
        else if (p.type === 'mat4') {
            return mat4_1.Mat4.areEqual(a, b, common_1.EPSILON);
        }
...
})(ParamDefinition || (exports.ParamDefinition = ParamDefinition = {}));
