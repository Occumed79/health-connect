"use strict";
/**
 * Copyright (c) 2026 mol* contributors, licensed under MIT, See LICENSE file for more info.
 *
 * @author Alexander Rose <alexander.rose@weirdbyte.de>
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.ObjShapeParams = void 0;
exports.createObjShapeParams = createObjShapeParams;
exports.shapeFromObj = shapeFromObj;
const mol_task_1 = require("../../mol-task/index.js");
const color_1 = require("../../mol-util/color/index.js");
const mesh_builder_1 = require("../../mol-geo/geometry/mesh/mesh-builder.js");
const mesh_1 = require("../../mol-geo/geometry/mesh/mesh.js");
const shape_1 = require("../../mol-model/shape.js");
const util_1 = require("../../mol-data/util.js");
const param_definition_1 = require("../../mol-util/param-definition.js");
const names_1 = require("../../mol-util/color/names.js");
const distinct_1 = require("../../mol-util/color/distinct.js");
const mol_util_1 = require("../../mol-util/index.js");
const debug_1 = require("../../mol-util/debug.js");
function createObjShapeParams(objFile, mtl) {
    var _a, _b;
    const hasVertexColors = ((_a = objFile === null || objFile === void 0 ? void 0 : objFile.vertexColors.length) !== null && _a !== void 0 ? _a : 0) > 0;
    const materialNames = (_b = objFile === null || objFile === void 0 ? void 0 : objFile.materialNames) !== null && _b !== void 0 ? _b : [];
    const hasMaterials = materialNames.length > 0;
    const hasMtl = mtl !== undefined && mtl.size > 0 && hasMaterials;
    const defaultColors = materialNames.length > 1
        ? (0, distinct_1.distinctColors)(materialNames.length)
        : materialNames.length === 1 ? [names_1.ColorNames.grey] : [];
    const materialColorParams = {};
    for (let i = 0; i < materialNames.length; ++i) {
        materialColorParams[materialNames[i]] = param_definition_1.ParamDefinition.Color(defaultColors[i]);
    }
    const coloringOptions = {
        uniform: param_definition_1.ParamDefinition.Group({
            color: param_definition_1.ParamDefinition.Color(names_1.ColorNames.grey),
        }, { isFlat: true }),
    };
    if (hasMtl) {
        coloringOptions.given = param_definition_1.ParamDefinition.Group({}, { isFlat: true });
    }
    coloringOptions.custom = param_definition_1.ParamDefinition.Group(materialColorParams, { isFlat: false });
    if (hasVertexColors) {
        coloringOptions.vertex = param_definition_1.ParamDefinition.Group({}, { isFlat: true });
    }
    const defaultColoring = hasVertexColors ? 'vertex' : hasMtl ? 'given' : hasMaterials ? 'custom' : 'uniform';
    return {
        ...mesh_1.Mesh.Params,
        coloring: param_definition_1.ParamDefinition.MappedStatic(defaultColoring, coloringOptions),
    };
}
exports.ObjShapeParams = createObjShapeParams();
/**
 * Resolve one Color per material group from the current params.
 * Returns an empty array when vertex-color mode is active (colors come from geometry).
 */
function getMaterialColors(materialNames, props, mtl) {
    const { coloring } = props;
    if (coloring.name === 'vertex')
        return [];
    const count = Math.max(1, materialNames.length);
    if (coloring.name === 'uniform') {
        return Array(count).fill(coloring.params.color);
    }
    else if (coloring.name === 'given') {
        // given: read Kd directly from the MTL file
        if (materialNames.length === 0)
            return [names_1.ColorNames.grey];
        return materialNames.map(name => { var _a, _b; return (_b = (_a = mtl === null || mtl === void 0 ? void 0 : mtl.get(name)) === null || _a === void 0 ? void 0 : _a.Kd) !== null && _b !== void 0 ? _b : names_1.ColorNames.grey; });
    }
    else {
        // custom: read one color per material name from user-editable params
        if (materialNames.length === 0)
            return [names_1.ColorNames.grey];
        const params = coloring.params;
        return materialNames.map(name => { var _a; return (_a = params[name]) !== null && _a !== void 0 ? _a : names_1.ColorNames.grey; });
    }
}
/**
 * Build a per-position → material-index lookup for use when the mesh is built
 * with position-index groups (i.e. when vertex colors are present).
 */
function buildPositionToMaterial(obj) {
    const map = new Int32Array(obj.positionCount);
    for (let t = 0; t < obj.triangleCount; ++t) {
        const triOffset = t * 3;
        const g = obj.faceGroups[t];
        for (let v = 0; v < 3; ++v) {
            map[obj.positionIndices[triOffset + v]] = g;
        }
    }
    return map;
}
function getCanUseIndexedPath(obj, usePositionGroups) {
    const { positionIndices, normalIndices, faceGroups, positionCount, triangleCount } = obj;
    const hasNormals = obj.normalCount > 0;
    let canUseIndexedPath = usePositionGroups;
    let posGroups = null;
    let posNormals = null;
    if (!usePositionGroups) {
        posGroups = new Int32Array(positionCount).fill(-1);
        let hasGroupConflict = false;
        outer: for (let t = 0; t < triangleCount; ++t) {
            const triOffset = t * 3;
            const g = faceGroups[t];
            for (let v = 0; v < 3; ++v) {
                const pi = positionIndices[triOffset + v];
                if (posGroups[pi] === -1) {
                    posGroups[pi] = g;
                }
                else if (posGroups[pi] !== g) {
                    hasGroupConflict = true;
                    break outer;
                }
            }
        }
        if (hasGroupConflict) {
            if (debug_1.isDebugMode)
                console.warn('OBJ: position shared across face groups; falling back to vertex duplication');
        }
        else {
            canUseIndexedPath = true;
        }
    }
    if (canUseIndexedPath && hasNormals) {
        posNormals = new Int32Array(positionCount).fill(-1);
        let hasNormalConflict = false;
        outerNormals: for (let t = 0; t < triangleCount; ++t) {
            const triOffset = t * 3;
            for (let v = 0; v < 3; ++v) {
                const pi = positionIndices[triOffset + v];
                const ni = normalIndices[triOffset + v];
                if (posNormals[pi] === -1) {
                    posNormals[pi] = ni;
                }
                else if (posNormals[pi] !== ni) {
                    hasNormalConflict = true;
                    break outerNormals;
                }
            }
        }
        if (hasNormalConflict) {
            canUseIndexedPath = false;
        }
    }
    return { canUseIndexedPath, posGroups, posNormals };
}
async function getIndexedMesh(ctx, obj, input, mesh) {
    const { positions, normals, positionIndices, triangleCount, positionCount } = obj;
    const hasNormals = obj.normalCount > 0;
    const { posGroups, posNormals, usePositionGroups } = input;
    const updateChunk = 50000;
    const builderState = mesh_builder_1.MeshBuilder.createState(positionCount, triangleCount, mesh);
    const { vertices, normals: normBuf, indices, groups } = builderState;
    for (let p = 0; p < positionCount; ++p) {
        const po = p * 3;
        util_1.ChunkedArray.add3(vertices, positions[po], positions[po + 1], positions[po + 2]);
        if (posNormals !== null && posNormals[p] >= 0) {
            const no = posNormals[p] * 3;
            util_1.ChunkedArray.add3(normBuf, normals[no], normals[no + 1], normals[no + 2]);
        }
        else {
            util_1.ChunkedArray.add3(normBuf, 0, 0, 0);
        }
        // position-index groups: group = position index; material groups: group = material idx
        util_1.ChunkedArray.add(groups, usePositionGroups ? p : Math.max(0, posGroups[p]));
        if (p % updateChunk === 0 && ctx.shouldUpdate) {
            await ctx.update({ message: 'Building OBJ mesh', current: p, max: positionCount });
        }
    }
    for (let t = 0; t < triangleCount; ++t) {
        const triOffset = t * 3;
        util_1.ChunkedArray.add3(indices, positionIndices[triOffset], positionIndices[triOffset + 1], positionIndices[triOffset + 2]);
    }
    const m = mesh_builder_1.MeshBuilder.getMesh(builderState);
    if (!hasNormals)
        mesh_1.Mesh.computeNormals(m);
    // TODO: check if needed
    mol_util_1.ValueCell.updateIfChanged(m.varyingGroup, true);
    return m;
}
async function getFaceMesh(ctx, obj, usePositionGroups, mesh) {
    const { positions, normals, positionIndices, normalIndices, triangleCount, faceGroups } = obj;
    const hasNormals = obj.normalCount > 0;
    const updateChunk = 50000;
    const builderState = mesh_builder_1.MeshBuilder.createState(triangleCount * 3, triangleCount, mesh);
    const { vertices, normals: normBuf, indices, groups } = builderState;
    for (let t = 0; t < triangleCount; ++t) {
        const triOffset = t * 3;
        const base = t * 3;
        for (let v = 0; v < 3; ++v) {
            const pi = positionIndices[triOffset + v];
            const po = pi * 3;
            util_1.ChunkedArray.add3(vertices, positions[po], positions[po + 1], positions[po + 2]);
            const ni = hasNormals ? normalIndices[triOffset + v] : -1;
            if (hasNormals && ni >= 0) {
                const no = ni * 3;
                util_1.ChunkedArray.add3(normBuf, normals[no], normals[no + 1], normals[no + 2]);
            }
            else {
                util_1.ChunkedArray.add3(normBuf, 0, 0, 0);
            }
            // position-index groups: group = position index; material groups: group = material idx
            util_1.ChunkedArray.add(groups, usePositionGroups ? pi : faceGroups[t]);
        }
        util_1.ChunkedArray.add3(indices, base, base + 1, base + 2);
        if (t % updateChunk === 0 && ctx.shouldUpdate) {
            await ctx.update({ message: 'Building OBJ mesh', current: t, max: triangleCount });
        }
    }
    const m = mesh_builder_1.MeshBuilder.getMesh(builderState);
    if (!hasNormals)
        mesh_1.Mesh.computeNormals(m);
    // TODO: check if needed
    mol_util_1.ValueCell.updateIfChanged(m.varyingGroup, true);
    return m;
}
/**
 * Build a mesh from indexed OBJ data.
 *
 * OBJ stores positions and normals as indexed arrays (each face-vertex
 * references a position index and an optional normal index). Mesh.create
 * requires a flat vertex array, so we expand the indexed data.
 *
 * When `usePositionGroups` is true (vertex colors present) the group ID for
 * each mesh vertex is set to the position index so the color function can
 * look up per-vertex colors directly.  Otherwise group IDs are material indices
 * (existing behaviour).
 */
async function getMesh(ctx, obj, usePositionGroups, mesh) {
    const { canUseIndexedPath, posGroups, posNormals } = getCanUseIndexedPath(obj, usePositionGroups);
    // Indexed path: map each position directly to a vertex (no duplication).
    // Conditions: (1) no position is shared across different face groups, and
    //             (2) if normals are present, each position maps to exactly one normal index.
    if (canUseIndexedPath) {
        return getIndexedMesh(ctx, obj, { posGroups, posNormals, usePositionGroups }, mesh);
    }
    // Fallback (position uses multiple normals, or material group-conflict detected): expand
    // indexed data into flat per-face-vertex arrays so each face-vertex can carry its own
    // normal/group.
    return getFaceMesh(ctx, obj, usePositionGroups, mesh);
}
function createShape(objData, mesh, colors, coloringName, positionToMaterial) {
    var _a;
    const { source, transforms } = objData;
    const { materialNames, vertexColors, positionCount } = source;
    const usePositionGroups = vertexColors.length > 0;
    let getColor;
    if (coloringName === 'vertex' && usePositionGroups) {
        getColor = (groupId) => color_1.Color.fromNormalizedArray(vertexColors, groupId * 3);
    }
    else if (usePositionGroups && positionToMaterial) {
        // material or uniform coloring with position-index groups
        if (coloringName === 'uniform') {
            const c = (_a = colors[0]) !== null && _a !== void 0 ? _a : names_1.ColorNames.grey;
            getColor = () => c;
        }
        else {
            getColor = (groupId) => colors[Math.min(positionToMaterial[groupId], colors.length - 1)];
        }
    }
    else {
        getColor = (groupId) => colors[Math.min(groupId, colors.length - 1)];
    }
    let getLabel;
    if (usePositionGroups && positionToMaterial && materialNames.length > 0) {
        getLabel = (groupId) => { var _a; return (_a = materialNames[positionToMaterial[groupId]]) !== null && _a !== void 0 ? _a : 'OBJ Mesh'; };
    }
    else if (!usePositionGroups && materialNames.length > 0) {
        getLabel = (groupId) => { var _a; return (_a = materialNames[groupId]) !== null && _a !== void 0 ? _a : 'OBJ Mesh'; };
    }
    else {
        getLabel = () => 'OBJ Mesh';
    }
    // When using position-index groups the group count equals positionCount;
    // otherwise pass colors.length (existing behaviour — let Shape derive from geometry
    // when colors.length is 0 to avoid a zero group count).
    const groupCount = usePositionGroups ? positionCount : (colors.length > 0 ? colors.length : undefined);
    return shape_1.Shape.create('obj-mesh', source, mesh, (groupId) => getColor(groupId), () => 1, (groupId) => getLabel(groupId), transforms, groupCount);
}
function makeShapeGetter() {
    let _objData;
    let _colors;
    let _coloringName;
    let _shape;
    let _mesh;
    let _positionToMaterial = null;
    const getShape = async (ctx, objData, props, shape) => {
        const newMesh = !_objData || _objData !== objData;
        const coloringName = props.coloring.name;
        const nextColors = getMaterialColors(objData.source.materialNames, props, objData.mtl);
        const newColor = !_colors
            || _coloringName !== coloringName
            || nextColors.length !== _colors.length
            || nextColors.some((c, i) => c !== _colors[i]);
        if (newMesh) {
            _colors = nextColors;
            _coloringName = coloringName;
            const usePositionGroups = objData.source.vertexColors.length > 0;
            _positionToMaterial = usePositionGroups ? buildPositionToMaterial(objData.source) : null;
            _mesh = await getMesh(ctx, objData.source, usePositionGroups, shape && shape.geometry);
            _shape = createShape(objData, _mesh, _colors, _coloringName, _positionToMaterial);
        }
        else if (newColor) {
            _colors = nextColors;
            _coloringName = coloringName;
            _shape = createShape(objData, _mesh, _colors, _coloringName, _positionToMaterial);
        }
        _objData = objData;
        return _shape;
    };
    return getShape;
}
function shapeFromObj(source, params) {
    return mol_task_1.Task.create('Shape Provider', async (_ctx) => {
        return {
            label: 'Mesh',
            data: { source, transforms: params === null || params === void 0 ? void 0 : params.transforms, mtl: params === null || params === void 0 ? void 0 : params.mtl },
            params: createObjShapeParams(source, params === null || params === void 0 ? void 0 : params.mtl),
            getShape: makeShapeGetter(),
            geometryUtils: mesh_1.Mesh.Utils,
        };
    });
}
