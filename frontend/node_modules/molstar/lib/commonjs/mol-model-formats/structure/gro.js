...
const trajectory_1 = require("../../mol-model/structure/trajectory.js");
    ...
    return mol_task_1.Task.create('Parse GRO', async (ctx) => {
        ...
        for (let i = 0, il = gro.structures.length; i < il; ++i) {
            ...
            if (m.frameCount === 1) {
                ...
                if (bx !== 0 && by !== 0 && bz !== 0) {
                    const symmetry = symmetry_1.ModelSymmetry.fromCell(linear_algebra_1.Vec3.create(bx * 10, by * 10, bz * 10), linear_algebra_1.Vec3.create(Math.PI / 2, Math.PI / 2, Math.PI / 2));
                    symmetry_1.ModelSymmetry.Provider.set(model, symmetry);
                }
                models.push(model);
            }
        }
