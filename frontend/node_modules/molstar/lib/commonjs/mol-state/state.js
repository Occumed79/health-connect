...
exports.StateTreeCycleError = exports.State = void 0;
const object_1 = require("./object.js");
const tree_1 = require("./tree.js");
const transform_1 = require("./transform.js");
const transformer_1 = require("./transformer.js");
const mol_task_1 = require("../mol-task/index.js");
const selection_1 = require("./state/selection.js");
const rx_event_helper_1 = require("../mol-util/rx-event-helper.js");
const builder_1 = require("./state/builder.js");
const manager_1 = require("./action/manager.js");
const log_entry_1 = require("../mol-util/log-entry.js");
const now_1 = require("../mol-util/now.js");
const param_definition_1 = require("../mol-util/param-definition.js");
const spine_1 = require("./tree/spine.js");
const async_queue_1 = require("../mol-util/async-queue.js");
const array_1 = require("../mol-util/array.js");
const unique_array_1 = require("../mol-data/generic/unique-array.js");
const object_2 = require("../mol-util/object.js");
/**
...
exports.StateTreeCycleError = StateTreeCycleError;
class State {
    ...
    dispose() {
        ...
        const refs = [];
        tree_1.StateTree.doPostOrder(this._tree, this._tree.root, { refs }, (n, _, s) => { s.refs.push(n.ref); });
        for (let i = refs.length - 1; i >= 0; i--) {
            const cell = this.cells.get(refs[i]);
            if (!cell)
                continue;
            try {
    ...
    constructor(rootObject, params) {
        ...
        this.tryGetCellData = (ref) => {
            var _a, _b;
            let ret = (_b = (_a = this.cells.get(ref)) === null || _a === void 0 ? void 0 : _a.obj) === null || _b === void 0 ? void 0 : _b.data;
            if (ret === undefined) {
...
function linkCells(target, ctx) {
    const effective = transform_1.StateTransform.getEffectiveDependsOn(target.transform);
 ...
 */
function relinkCells(target, ctx) {
    const effective = transform_1.StateTransform.getEffectiveDependsOn(target.transform);
    ...
    if (current.length === effective.length) {
        ...
        for (const c of current) {
            if (effective.indexOf(c.transform.ref) < 0) {
                same = false;
                break;
            }
        }
    ...
    for (let i = current.length - 1; i >= 0; i--) {
        ...
        if (!desired.has(dep.transform.ref)) {
            current.splice(i, 1);
            (0, array_1.arraySetRemove)(dep.dependencies.dependentBy, target);
            changed = true;
        }
    }
 ...
 */
function detectDependenciesCycle(start) {
    if (start.dependencies.dependsOn.length === 0)
        return void 0;
    const stack = [{ cell: start, idx: 0 }];
    const onPath = new Set([start.transform.ref]);
    const fully = new Set();
...
function checkDependenciesCycle(cell) {
    const cycle = detectDependenciesCycle(cell);
    if (cycle)
        throw new StateTreeCycleError(cycle);
}
    ...
    initCtx.visited.forEach(ref => {
        ...
        for (const by of cell.dependencies.dependentBy) {
            if (initCtx.visited.has(by.transform.ref))
                continue;
            if (!dependent)
                dependent = unique_array_1.UniqueArray.create();
            unique_array_1.UniqueArray.add(dependent, by.transform.ref, by);
        }
    });
...
function collectScheduledVisitor(t, _, s) {
    s.add(t.ref);
}
function collectScheduled(ctx, roots) {
    const out = new Set();
    for (const root of roots) {
        const node = ctx.tree.transforms.get(root);
        if (!node)
            continue;
        tree_1.StateTree.doPreOrder(ctx.tree, node, out, collectScheduledVisitor);
    }
    return out;
}
/**
 ...
 */
function pendingBlockers(ctx, cell) {
    const blockers = [];
    ...
    for (const dep of cell.dependencies.dependsOn) {
        ...
        if (dep.status === 'error')
            continue;
        if (!scheduled.has(dep.transform.ref))
            continue;
