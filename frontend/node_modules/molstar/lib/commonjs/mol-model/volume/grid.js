...
(function (Grid) {
    ...
    const _isoBbox = (0, geometry_1.Box3D)();
    function getIsosurfaceBoundingSphere(grid, isoValue, boundingSphere) {
        ...
        linear_algebra_1.Vec3.set(_isoBbox.min, minx - 1, miny - 1, minz - 1);
        linear_algebra_1.Vec3.set(_isoBbox.max, maxx + 1, maxy + 1, maxz + 1);
        const transform = Grid.getGridToCartesianTransform(grid);
        geometry_1.Box3D.transform(_isoBbox, _isoBbox, transform);
        return geometry_1.Sphere3D.fromBox3D(boundingSphere || (0, geometry_1.Sphere3D)(), _isoBbox);
    }
    Grid.getIsosurfaceBoundingSphere = getIsosurfaceBoundingSphere;
    /**
     * Compute histogram with given bin count.
     * Cached on the Grid object.
     */
    function getHistogram(grid, binCount) {
        let histograms = grid._histograms;
        if (!histograms) {
            histograms = grid._histograms = {};
        }
     ...
     * Cached on the Grid object.
     */
    function getRobustStats(grid, opts) {
        var _e;
        const binCount = (_e = opts === null || opts === void 0 ? void 0 : opts.binCount) !== null && _e !== void 0 ? _e : Grid.RobustStatsBinCount;
        const ignoreZero = !!(opts === null || opts === void 0 ? void 0 : opts.ignoreZero);
        const key = `${binCount}|${ignoreZero ? 1 : 0}`;
        let cache = grid._robustStats;
        if (!cache)
            cache = grid._robustStats = {};
        if (!cache[key]) {
            cache[key] = (0, histogram_1.histogramRobustStats)(getHistogram(grid, binCount), { ignoreZero });
        }
        return cache[key];
    }
    Grid.getRobustStats = getRobustStats;
