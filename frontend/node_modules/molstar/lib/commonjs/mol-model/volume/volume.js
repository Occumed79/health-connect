...
(function (Volume) {
    ...
    function getBoundingSphere(volume, instances, boundingSphere) {
        const gs = grid_1.Grid.getBoundingSphere(volume.grid);
        if (!boundingSphere)
            boundingSphere = (0, geometry_1.Sphere3D)();
        if (int_1.OrderedSet.isEmpty(instances))
            return geometry_1.Sphere3D.copy(boundingSphere, gs);
        const spheres = [];
        for (let i = 0, il = int_1.OrderedSet.size(instances); i < il; ++i) {
            const { transform } = volume.instances[int_1.OrderedSet.getAt(instances, i)];
    ...
    (function (Isosurface) {
        ...
        const boundaryHelper = new boundary_helper_1.BoundaryHelper('98');
        function getBoundingSphere(volume, isoValue, instances, boundingSphere) {
            const value = Volume.IsoValue.toAbsolute(isoValue, volume.grid.stats).absoluteValue;
            const gs = grid_1.Grid.getIsosurfaceBoundingSphere(volume.grid, value);
            if (int_1.OrderedSet.isEmpty(instances))
                return geometry_1.Sphere3D.copy(boundingSphere || (0, geometry_1.Sphere3D)(), gs);
            const spheres = [];
            for (let i = 0, il = int_1.OrderedSet.size(instances); i < il; ++i) {
                spheres.push(geometry_1.Sphere3D.transform((0, geometry_1.Sphere3D)(), gs, volume.instances[int_1.OrderedSet.getAt(instances, i)].transform));
            }
            boundaryHelper.reset();
            for (const s of spheres)
                boundaryHelper.includeSphere(s);
    ...
    (function (Segment) {
        ...
        function getBoundingSphere(volume, elements, boundingSphere) {
            ...
            if (segmentation) {
                ...
                return Volume.getBoundingSphere(volume, int_1.Interval.ofLength(volume.instances.length), boundingSphere);
            }
        }
