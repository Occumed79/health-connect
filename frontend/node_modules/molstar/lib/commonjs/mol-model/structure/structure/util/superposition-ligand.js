...
exports.DefaultLigandMccsOptions = void 0;
exports.isSingleLigandLoci = isSingleLigandLoci;
exports.buildLigandGraphFromLoci = buildLigandGraphFromLoci;
exports.matchLigandsByAtomName = matchLigandsByAtomName;
exports.findMccsCliques = findMccsCliques;
exports.findMccsPairs = findMccsPairs;
exports.superposeLigandsByMccs = superposeLigandsByMccs;
const int_1 = require("../../../../mol-data/int.js");
const types_1 = require("../../model/types.js");
const graph_1 = require("../../../../mol-math/graph.js");
const mccs_1 = require("../../../../mol-math/graph/mccs.js");
const linear_algebra_1 = require("../../../../mol-math/linear-algebra.js");
const minimize_rmsd_1 = require("../../../../mol-math/linear-algebra/3d/minimize-rmsd.js");
const structure_1 = require("../../structure.js");
exports.DefaultLigandMccsOptions = {
    ...
    edgeTest: (orderA, flagsA, orderB, flagsB) => {
        ...
        const aArom = (flagsA & types_1.BondType.Flag.Aromatic) !== 0;
        const bArom = (flagsB & types_1.BondType.Flag.Aromatic) !== 0;
        if (aArom || bArom)
            return aArom === bArom;
...
function isAtomicUnit(u) {
    return u.kind === structure_1.Unit.Kind.Atomic;
}
/**
 ...
 */
function isSingleLigandLoci(loci) {
    if (structure_1.StructureElement.Loci.isEmpty(loci))
        return;
    const loc = structure_1.StructureElement.Location.create(loci.structure);
    const first = structure_1.StructureElement.Loci.getFirstLocation(loci, loc);
    if (!first)
        return;
    if (structure_1.StructureProperties.entity.type(first) === 'polymer')
        return;
    const firstResidueKey = structure_1.StructureProperties.residue.key(first);
    const firstCompId = structure_1.StructureProperties.atom.label_comp_id(first);
    let ok = true;
    structure_1.StructureElement.Loci.forEachLocation(loci, l => {
        ...
        if (structure_1.StructureProperties.entity.type(l) === 'polymer')
            ok = false;
        if (structure_1.StructureProperties.residue.key(l) !== firstResidueKey)
            ok = false;
    });
    if (!ok)
        return;
...
function buildLigandGraphFromLoci(loci) {
    ...
    if (indices.length === 0)
        return;
    const loc = structure_1.StructureElement.Location.create(loci.structure, unit);
    for (let i = 0; i < indices.length; i++) {
        loc.element = indices[i];
        if (structure_1.StructureProperties.residue.key(loc) !== residueKey)
            return;
    }
    indices.sort((a, b) => a - b);
    // intra-unit bonds are keyed by unit-local UnitIndex, not the global ElementIndex, so track both
    const unitIndices = new Array(indices.length);
    const unitIndexToVertex = new Map();
    const vertices = indices.map((el, i) => {
        loc.element = el;
        const uIndex = int_1.SortedArray.indexOf(unit.elements, el);
        unitIndices[i] = uIndex;
        unitIndexToVertex.set(uIndex, i);
        return {
            index: i,
            element: el,
            atomName: structure_1.StructureProperties.atom.label_atom_id(loc),
            elementSymbol: structure_1.StructureProperties.atom.type_symbol(loc)
        };
    });
    ...
    for (let vi = 0; vi < vertices.length; vi++) {
        ...
        for (let ei = offset[uIndex], end = offset[uIndex + 1]; ei < end; ei++) {
            ...
            if (vj === void 0 || vj <= vi)
                continue; // neighbor outside the ligand, or already added (vj < vi)
            xs.push(vi);
            ys.push(vj);
            orders.push(srcOrder[ei]);
            flags.push(srcFlags[ei]);
        }
    }
    const builder = new graph_1.IntAdjacencyGraph.EdgeBuilder(vertices.length, xs, ys);
...
function matchLigandsByAtomName(a, b) {
    if (!a.compId || !b.compId)
        return;
    if (a.compId.toUpperCase() !== b.compId.toUpperCase())
        return;
    ...
    for (const va of a.vertices) {
        ...
        for (let i = 0; i < cands.length; i++) {
            ...
            if (usedB[bi])
                continue;
            if (b.vertices[bi].elementSymbol === va.elementSymbol) {
                chosen = bi;
                break;
            }
        }
 ...
 */
function findMccsCliques(a, b, options = {}, status) {
    const opts = { ...exports.DefaultLigandMccsOptions, ...options };
    ...
    return mccs_1.Mccs.find(a.bonds, b.bonds, {
        vertexTest: (ia, ib) => {
            if (ignoreH && (isHydrogenVertex(a.vertices[ia]) || isHydrogenVertex(b.vertices[ib])))
                return false;
...
function findMccsPairs(a, b, options = {}) {
...
function fillPairPositions(gA, gB, pairs, posA, posB) {
...
function pairsToElements(gA, gB, pairs) {
    const referenceElements = new Array(pairs.length);
    const targetElements = new Array(pairs.length);
    for (let i = 0; i < pairs.length; i++) {
 ...
 */
function superposeLigandsByMccs(reference, target, options = {}) {
    const opts = { ...exports.DefaultLigandMccsOptions, ...options };
    ...
    if (namePairs && namePairs.length >= opts.minMatchedAtoms) {
        ...
        const posA = minimize_rmsd_1.MinimizeRmsd.Positions.empty(n);
        const posB = minimize_rmsd_1.MinimizeRmsd.Positions.empty(n);
        fillPairPositions(gA, gB, namePairs, posA, posB);
        const { bTransform, rmsd } = minimize_rmsd_1.MinimizeRmsd.compute({ a: posA, b: posB, length: n });
    ...
    const posA = minimize_rmsd_1.MinimizeRmsd.Positions.empty(n);
    const posB = minimize_rmsd_1.MinimizeRmsd.Positions.empty(n);
    const result = { bTransform: linear_algebra_1.Mat4.zero(), rmsd: 0, nAlignedElements: 0 };
    const state = new minimize_rmsd_1.RmsdTransformState({ a: posA, b: posB, length: n }, result);
    let bestRmsd = Number.POSITIVE_INFINITY;
    let bestPairs;
    const bestTransform = linear_algebra_1.Mat4.zero();
    for (const pairs of cliques) {
        fillPairPositions(gA, gB, pairs, posA, posB);
        minimize_rmsd_1.MinimizeRmsd.compute({ a: posA, b: posB, length: n }, result, state);
        if (result.rmsd < bestRmsd) {
            bestRmsd = result.rmsd;
            linear_algebra_1.Mat4.copy(bestTransform, result.bTransform);
            bestPairs = pairs;
        }
    }
    if (!bestPairs)
