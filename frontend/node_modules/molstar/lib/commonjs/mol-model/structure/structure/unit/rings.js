...
(function (UnitRing) {
    ...
    const Sp3RingCheckElements = new Set([
        types_2.Elements.B, types_2.Elements.C, types_2.Elements.N,
        types_2.Elements.SI, types_2.Elements.P, types_2.Elements.S,
        types_2.Elements.GE, types_2.Elements.AS,
        types_2.Elements.SN, types_2.Elements.SB,
        types_2.Elements.BI
    ...
    function isAromatic(unit, ring) {
        ...
        for (let i = 0, il = ring.length; i < il; ++i) {
            ...
            if (!Sp3RingCheckElements.has(elem))
                continue;
            let degree = 0;
            let hasPiBond = false;
            for (let j = offset[aI], jl = offset[aI + 1]; j < jl; ++j) {
                ...
                const f = flags[j];
                const o = order[j];
                if (types_1.BondType.is(types_1.BondType.Flag.Aromatic, f) || o === 2 || o === 3) {
                    hasPiBond = true;
                }
            }
            if (degree >= 4 && !hasPiBond)
                return false;
        }
