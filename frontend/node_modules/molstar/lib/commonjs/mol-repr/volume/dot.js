...
function createVolumeSphereImpostor(ctx, volume, key, theme, props, spheres) {
    ...
    const add = (x, y, z) => {
        ...
        if (basis) {
            vec3_1.Vec3.add(p, p, getRandomOffsetFromBasis(basis));
        }
    ...
    if (useMortonOrder) {
        ...
        let size = 1;
        while (size < xn || size < yn || size < zn)
            size <<= 1;
        const visit = (x0, y0, z0, s) => {
    ...
    s.setBoundingSphere(volume_1.Grid.getIsosurfaceBoundingSphere(volume.grid, volume_1.Volume.IsoValue.toAbsolute(props.isoValue, volume.grid.stats).absoluteValue));
    return s;
}
function createVolumeSphereMesh(ctx, volume, key, theme, props, mesh) {
    ...
    m.setBoundingSphere(volume_1.Grid.getIsosurfaceBoundingSphere(volume.grid, volume_1.Volume.IsoValue.toAbsolute(props.isoValue, volume.grid.stats).absoluteValue));
    return m;
}
//
...
function createVolumePoint(ctx, volume, key, theme, props, points) {
    ...
    pt.setBoundingSphere(volume_1.Grid.getIsosurfaceBoundingSphere(volume.grid, volume_1.Volume.IsoValue.toAbsolute(props.isoValue, volume.grid.stats).absoluteValue));
    return pt;
}
//
