...
exports.KinParser = void 0;
const hsv_1 = require("../../../mol-util/color/spaces/hsv.js");
const ColorDict = {
    red: hsv_1.Hsv.toColor(hsv_1.Hsv.fromArray([0, 100, 100])),
    orange: hsv_1.Hsv.toColor(hsv_1.Hsv.fromArray([20, 100, 100])),
    gold: hsv_1.Hsv.toColor(hsv_1.Hsv.fromArray([40, 100, 100])),
    yellow: hsv_1.Hsv.toColor(hsv_1.Hsv.fromArray([60, 100, 100])),
    lime: hsv_1.Hsv.toColor(hsv_1.Hsv.fromArray([80, 100, 100])),
    green: hsv_1.Hsv.toColor(hsv_1.Hsv.fromArray([120, 80, 100])),
    sea: hsv_1.Hsv.toColor(hsv_1.Hsv.fromArray([150, 100, 100])),
    cyan: hsv_1.Hsv.toColor(hsv_1.Hsv.fromArray([180, 100, 85])),
    sky: hsv_1.Hsv.toColor(hsv_1.Hsv.fromArray([210, 75, 95])),
    blue: hsv_1.Hsv.toColor(hsv_1.Hsv.fromArray([240, 70, 100])),
    purple: hsv_1.Hsv.toColor(hsv_1.Hsv.fromArray([275, 75, 100])),
    magenta: hsv_1.Hsv.toColor(hsv_1.Hsv.fromArray([300, 95, 100])),
    hotpink: hsv_1.Hsv.toColor(hsv_1.Hsv.fromArray([335, 100, 100])),
    pink: hsv_1.Hsv.toColor(hsv_1.Hsv.fromArray([350, 55, 100])),
    peach: hsv_1.Hsv.toColor(hsv_1.Hsv.fromArray([25, 75, 100])),
    lilac: hsv_1.Hsv.toColor(hsv_1.Hsv.fromArray([275, 55, 100])),
    pinktint: hsv_1.Hsv.toColor(hsv_1.Hsv.fromArray([340, 30, 100])),
    peachtint: hsv_1.Hsv.toColor(hsv_1.Hsv.fromArray([25, 50, 100])),
    yellowtint: hsv_1.Hsv.toColor(hsv_1.Hsv.fromArray([60, 50, 100])),
    greentint: hsv_1.Hsv.toColor(hsv_1.Hsv.fromArray([135, 40, 100])),
    bluetint: hsv_1.Hsv.toColor(hsv_1.Hsv.fromArray([220, 40, 100])),
    lilactint: hsv_1.Hsv.toColor(hsv_1.Hsv.fromArray([275, 35, 100])),
    white: hsv_1.Hsv.toColor(hsv_1.Hsv.fromArray([0, 0, 100])),
    gray: hsv_1.Hsv.toColor(hsv_1.Hsv.fromArray([0, 0, 50])),
    brown: hsv_1.Hsv.toColor(hsv_1.Hsv.fromArray([20, 45, 75])),
    deadwhite: hsv_1.Hsv.toColor(hsv_1.Hsv.fromArray([0, 0, 100])),
    deadblack: hsv_1.Hsv.toColor(hsv_1.Hsv.fromArray([0, 0, 0])),
    invisible: hsv_1.Hsv.toColor(hsv_1.Hsv.fromArray([0, 0, 0]))
};
const reWhitespaceComma = /[\s,]+/;
const reCurlyWhitespace = /[^{}\s]*{[^{}]+}|[^{}\s]+/g;
const reTrimCurly = /^{+|}+$/g;
const reCollapseEqual = /\s*=\s*/g;
function parseListDef(line, localColorDict) {
    let name;
    let defaultColor = localColorDict['white']; // Default color is white, but it can be overridden by the list definition
    let radius;
    let nobutton = false;
    const master = [];
...
function parseListElm(line, localColorDict) {
    line = line.trim();
    ...
    for (let lsindex = 4; lsindex <= ls.length; lsindex++) {
        ...
        if (literal.startsWith("'") && literal.endsWith("'")) {
            ...
            const tagString = literal.substring(1, literal.length - 1);
...
function parsePointmaster(line) {
    let name = '';
    const tags = [];
    let on = undefined;
...
class KinParser {
    ...
    _parse(data) {
        ...
        const localColorDict = Object.assign({}, ColorDict);
        let currentGroup = '';
        ...
        let prevDotLabel = '';
        let dotDefaultColor;
        let dotLabel, dotPosition, dotColor, dotPointMasters;
        ...
        let vecWidth, vecPointMasters;
        let isBallList = false;
        let prevBallLabel = '';
        let ballRadius, ballDefaultColor, ballDefaultRadius;
        let ballLabel, ballPosition, ballColor, ballPointMasters;
        let isRibbonList = false;
        let ribbonIsTriangles = false;
        let prevRibbonPointLabel = '';
        let ribbonListDefaultColor = localColorDict['white'];
        let ribbonPointLabelArray, ribbonPointPositionArray, ribbonPointBreakArray, ribbonPointColorArray;
        let ribbonPointMasters;
        let isText = false;
        let isCaption = false;
                ...
                if (!line) {
                    ...
                    if (!kinemage.groupDict[groupName]) {
                        ...
                        if (groupFlags['animate']) {
                            foundAnimate = true;
                            kinemage.groupsAnimate.push(groupName);
                            kinemage.activeAnimateGroup = 0;
                        }
                        if (groupFlags['2animate']) {
                            found2Animate = true;
                            kinemage.groupsAnimate2.push(groupName);
                    ...
                    for (const key in groupFlags) {
                        kinemage.groupDict[groupName][key] = groupFlags[key];
                    }
                }
                else if (line.startsWith('@subgroup')) {
                    const { groupName, groupFlags, groupMasters } = parseGroup(line);
                    const combinedName = currentGroup + ':' + groupName;
                    ...
                    for (const key in groupFlags) {
                        kinemage.subgroupDict[combinedName][key] = groupFlags[key];
                    }
                }
                    ...
                    if (!kinemage.viewDict[viewCount])
                        kinemage.viewDict[viewCount] = {};
