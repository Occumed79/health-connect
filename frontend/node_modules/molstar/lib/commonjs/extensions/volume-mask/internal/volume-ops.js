...
exports.flipVolumeX = flipVolumeX;
 ...
 */
function flipVolumeX(cells, nx, ny, nz, space) {
 ...
 */
function removeDust(cells, nx, ny, nz, space, minVoxels, threshold) {
        ...
        for (let iy = 0; iy < ny; iy++) {
            for (let ix = 0; ix < nx; ix++) {
                ...
                while (stack.length > 0) {
                    ...
                    const cy = stack.pop();
                    const cx = stack.pop();
                    const tryNeighbor = (nx2, ny2, nz2) => {
                        const noff = space.dataOffset(nx2, ny2, nz2);
                        if (visited[noff] || cells[noff] < threshold)
                            return;
                        visited[noff] = 1;
                        component.push(noff);
