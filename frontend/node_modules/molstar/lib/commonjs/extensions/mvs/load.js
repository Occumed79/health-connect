...
async function _loadMVS(ctx, plugin, data, options = {}) {
    ...
    try {
        ...
        if (entries.length > 0) {
            let indexToApply = (_a = options.defaultSnapshotIndex) !== null && _a !== void 0 ? _a : 0;
            if (indexToApply < 0 || indexToApply >= entries.length) {
                indexToApply = 0;
            }
            await commands_1.PluginCommands.State.Snapshots.Apply(plugin, { id: entries[indexToApply].snapshot.id });
        }
    }
    catch (err) {
