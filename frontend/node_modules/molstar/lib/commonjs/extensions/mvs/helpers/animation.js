...
exports.generateStateTransition = generateStateTransition;
const int_1 = require("../../../mol-data/int.js");
const easing_1 = require("../../../mol-math/easing.js");
...
async function generateStateTransition(ctx, snapshot, snapshotIndex, snapshotCount) {
    var _a, _b, _c, _d, _e;
    if (!snapshot.animation)
        return undefined;
    const tree = (0, tree_utils_1.addDefaults)(snapshot.animation, animation_tree_1.MVSAnimationSchema);
    ...
    for (let i = 0; i <= N; i++) {
        ...
        if (root === prevRoot || (prevRoot && (0, mol_util_1.deepEqual)(root, prevRoot))) {
...
const EasingFnMap = easing_1.EasingFunctions;
...
function createSnapshot(tree, transitionGroups, time, cache, nodeMap) {
    let modified = false;
    const ret = (0, produce_1.produce)(tree, (draft) => {
        var _a, _b, _c;
        for (const transitionGroup of transitionGroups) {
            ...
            if (transition.params.kind === 'transform_matrix') {
                next = processTransformMatrix(transition, target, (0, interpolate_1.clamp)(t, 0, 1), cacheEntry, offset, previous);
            }
            else {
...
function applyFrequency(t, frequency, alternate) {
    ...
    if (Math.abs(v - 1) < linear_algebra_1.EPSILON)
        return 1;
    v = v % 2;
    if (v > 1)
        return 2 - v;
...
function processScalarLike(transition, target, time, cacheEntry, offset, previous) {
    ...
    if (time <= 0)
        return startValue;
    else if (time >= 1 - linear_algebra_1.EPSILON && !transition.params.alternate_direction && transition.params.kind !== 'color')
        return endValue;
    let t = (0, interpolate_1.clamp)(time, 0, 1);
...
const TransformState = {
    pivotTranslation: (0, linear_algebra_1.Mat4)(),
    pivotTranslationInv: (0, linear_algebra_1.Mat4)(),
    rotation: (0, linear_algebra_1.Mat4)(),
    scale: (0, linear_algebra_1.Mat4)(),
    translation: (0, linear_algebra_1.Mat4)(),
    pivotNeg: (0, linear_algebra_1.Vec3)(),
    temp: (0, linear_algebra_1.Mat4)(),
};
function processTransformMatrix(transition, target, time, cache, offset, previous) {
    ...
    if (previous && previous.params.kind !== 'transform_matrix')
        return;
    const transform = (_a = select(target, transition.params.property, offset)) !== null && _a !== void 0 ? _a : linear_algebra_1.Mat4.identity();
    const startRotation = (_c = (_b = transition.params.rotation_start) !== null && _b !== void 0 ? _b : getPreviousMatrixEnd(previous, 'rotation_start')) !== null && _c !== void 0 ? _c : linear_algebra_1.Mat3.fromMat4((0, linear_algebra_1.Mat3)(), transform);
    const startTranslation = (_e = (_d = transition.params.translation_start) !== null && _d !== void 0 ? _d : getPreviousMatrixEnd(previous, 'translation_start')) !== null && _e !== void 0 ? _e : linear_algebra_1.Mat4.getTranslation((0, linear_algebra_1.Vec3)(), transform);
    const startScale = (_g = (_f = transition.params.scale_start) !== null && _f !== void 0 ? _f : getPreviousMatrixEnd(previous, 'scale_start')) !== null && _g !== void 0 ? _g : linear_algebra_1.Mat4.getScaling((0, linear_algebra_1.Vec3)(), transform);
    ...
    if (time <= 0) {
        ...
        const clampedTime = (0, interpolate_1.clamp)(time, 0, 1);
    ...
    const pivot = (_v = transition.params.pivot) !== null && _v !== void 0 ? _v : linear_algebra_1.Vec3.zero();
    linear_algebra_1.Mat4.fromTranslation(TransformState.translation, translation);
    linear_algebra_1.Mat4.fromScaling(TransformState.scale, scale);
    linear_algebra_1.Mat4.setIdentity(TransformState.rotation);
    linear_algebra_1.Mat4.fromMat3(TransformState.rotation, rotation);
    linear_algebra_1.Mat4.fromTranslation(TransformState.pivotTranslation, pivot);
    linear_algebra_1.Mat4.fromTranslation(TransformState.pivotTranslationInv, linear_algebra_1.Vec3.negate(TransformState.pivotNeg, pivot));
    // translation . pivot . rotation . scale . pivotInv
    const result = (0, linear_algebra_1.Mat4)();
    linear_algebra_1.Mat4.mul(result, TransformState.scale, TransformState.pivotTranslationInv);
    linear_algebra_1.Mat4.mul(result, TransformState.rotation, result);
    linear_algebra_1.Mat4.mul(result, TransformState.translation, result);
    return result;
}
...
function interpolateScalar(start, end, t, noise, discrete) {
    let v = typeof end === 'number' ? (0, interpolate_1.lerp)(start, end, t) : start;
    if (noise) {
...
function interpolateVectors(start, end, t, noise, isSpherical) {
    ...
    for (let i = 0; i < start.length; i += 3) {
        const s = linear_algebra_1.Vec3.fromArray(InterpolateVectorsState.start, start, i);
        let v;
        if (end) {
            const e = linear_algebra_1.Vec3.fromArray(InterpolateVectorsState.end, end, i);
            v = isSpherical
                ? linear_algebra_1.Vec3.slerp(InterpolateVectorsState.v, s, e, t)
                : linear_algebra_1.Vec3.lerp(InterpolateVectorsState.v, s, e, t);
        }
        else {
            v = linear_algebra_1.Vec3.clone(s);
        }
        if (noise && t <= 1 - linear_algebra_1.EPSILON) {
            linear_algebra_1.Vec3.random(Vec3Noise, noise);
            linear_algebra_1.Vec3.add(v, v, Vec3Noise);
        }
        linear_algebra_1.Vec3.toArray(v, ret, i);
    }
    return ret;
}
const Vec3Noise = (0, linear_algebra_1.Vec3)();
function interpolateVec3(start, end, t, noise, isSpherical) {
    ...
    if (end) {
        v = isSpherical
            ? linear_algebra_1.Vec3.slerp((0, linear_algebra_1.Vec3)(), start, end, t)
            : linear_algebra_1.Vec3.lerp((0, linear_algebra_1.Vec3)(), start, end, t);
    }
    else {
        v = linear_algebra_1.Vec3.clone(start);
    }
    if (noise && t <= 1 - linear_algebra_1.EPSILON) {
        linear_algebra_1.Vec3.random(Vec3Noise, noise);
        linear_algebra_1.Vec3.add(v, v, Vec3Noise);
...
const RotationState = {
    start: (0, linear_algebra_1.Quat)(),
    end: (0, linear_algebra_1.Quat)(),
    v: (0, linear_algebra_1.Quat)(),
    noise: (0, linear_algebra_1.Quat)(),
    axis: (0, linear_algebra_1.Vec3)(),
    temp: (0, linear_algebra_1.Mat4)(),
};
    ...
    if (end) {
        if (!cache.rotation) {
            cache.rotation = {
                ...
                start: linear_algebra_1.Quat.fromMat3((0, linear_algebra_1.Quat)(), start),
                end: linear_algebra_1.Quat.fromMat3((0, linear_algebra_1.Quat)(), end),
            };
        }
        const { axis, angle, start: startQ, end: endQ } = cache.rotation;
        if (angle < 1e-6) {
            // start ≈ end: make a clean spin about the detected (or default) axis
            linear_algebra_1.Quat.setAxisAngle(RotationState.v, axis, t * 2 * Math.PI); // Make a full turn
        }
        else {
            // Normal case: stick with your existing slerp between start/end
            linear_algebra_1.Quat.slerp(RotationState.v, startQ, endQ, t);
        }
    }
    else {
        linear_algebra_1.Quat.fromMat3(RotationState.v, start);
    }
    if (noise && t <= 1 - linear_algebra_1.EPSILON) {
        linear_algebra_1.Vec3.random(RotationState.axis, 1);
        linear_algebra_1.Quat.setAxisAngle(RotationState.noise, RotationState.axis, 2 * Math.PI * noise * (Math.random() - 0.5));
        linear_algebra_1.Quat.multiply(RotationState.v, RotationState.noise, RotationState.v);
    }
    linear_algebra_1.Mat4.fromQuat(RotationState.temp, RotationState.v);
    return linear_algebra_1.Mat3.fromMat4((0, linear_algebra_1.Mat3)(), RotationState.temp);
}
function decodeColors(color, baseColors) {
    ...
    if (typeof color === 'object') {
        ...
        if (baseColors) {
            for (const key of Object.keys(baseColors)) {
                const decoded = (0, utils_1.decodeColor)(baseColors[key]);
        ...
        for (const key of Object.keys(color)) {
            const decoded = (0, utils_1.decodeColor)(color[key]);
            if (decoded !== undefined) {
                ret[key] = decoded;
            }
        }
    ...
    return (0, utils_1.decodeColor)(color);
}
function interpolateColors(start, end, time, cacheEntry, baseColors) {
    const t = (0, interpolate_1.clamp)(time, 0, 1);
    if (cacheEntry.paletteFn) {
        const c = cacheEntry.paletteFn(t);
        return color_1.Color.toHexStyle(c);
    }
    ...
    if (typeof startColor === 'object') {
        ...
        if (typeof endColor === 'object') {
            for (const key of Object.keys(endColor)) {
                ret[key] = color_1.Color.toHexStyle(color_1.Color.interpolate(startColor[key], endColor[key], t));
            }
        }
        else if (typeof endColor === 'number') {
            for (const key of Object.keys(startColor)) {
                ret[key] = color_1.Color.toHexStyle(color_1.Color.interpolate(startColor[key], endColor, t));
            }
        }
        return ret;
    }
    ...
    if (typeof endColor === 'number' && typeof startColor === 'number') {
        return color_1.Color.toHexStyle(color_1.Color.interpolate(startColor, endColor, t));
    }
    return start;
}
...
function makePaletteFunction(props) {
    if (props.params.kind !== 'color' || !props.params.palette)
        return undefined;
    const params = (0, load_helpers_1.palettePropsFromMVSPalette)(props.params.palette);
...
function makePaletteFunctionDiscrete(props) {
    const defaultColor = (0, color_1.Color)(0x0);
    if (props.colors.length === 0)
        return () => defaultColor;
    return (value) => {
        const x = (0, interpolate_1.clamp)(value, 0, 1);
...
function makePaletteFunctionContinuous(props) {
    const defaultColor = (0, color_1.Color)(0x0);
    const { colors, checkpoints } = (0, annotation_color_theme_1.makeContinuousPaletteCheckpoints)(props);
    if (colors.length === 0)
        return () => defaultColor;
...
const RelativeAxisAngleState = {
    Rt: (0, linear_algebra_1.Mat3)(),
    R: (0, linear_algebra_1.Mat3)(),
};
function relativeAxisAngle(start, end) {
    // R_rel = end * start^T
    const R0 = start, R1 = end;
    const Rt = linear_algebra_1.Mat3.transpose(RelativeAxisAngleState.Rt, R0);
    const R = linear_algebra_1.Mat3.mul(RelativeAxisAngleState.R, R1, Rt);
    const tr = R[0] + R[4] + R[8]; // trace
    let angle = Math.acos((0, interpolate_1.clamp)((tr - 1) * 0.5, -1, 1)); // in [0, π]
    const axis = (0, linear_algebra_1.Vec3)();
    const eps = 1e-6;
    const sinA = Math.sin(angle);
    if (angle < eps) {
        // Near identity: axis undefined; return any unit axis (choose something stable)
        linear_algebra_1.Vec3.set(axis, 0, 0, 1);
        angle = 0.0;
        return { axis, angle };
    }
    ...
    if (x >= y && x >= z) {
        ...
        y = (R[1] + R[3]) / (4 * x);
        z = (R[2] + R[6]) / (4 * x);
        linear_algebra_1.Vec3.set(axis, x, y, z);
    }
    else if (y >= x && y >= z) {
        y = Math.max(y, 1e-8);
        x = (R[1] + R[3]) / (4 * y);
