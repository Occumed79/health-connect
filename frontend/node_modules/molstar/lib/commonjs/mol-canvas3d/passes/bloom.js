...
const composite_frag_1 = require("../../mol-gl/shader/bloom/composite.frag.js");
const luminosity_frag_1 = require("../../mol-gl/shader/bloom/luminosity.frag.js");
const blur_frag_1 = require("../../mol-gl/shader/bloom/blur.frag.js");
...
exports.BloomParams = {
    ...
    transparency: param_definition_1.ParamDefinition.Boolean(true, { description: 'Include transparent objects in emissive bloom', hideIf: p => p.mode !== 'emissive' }),
};
class BloomPass {
    ...
    getByteCount() {
        return (this.emissiveTarget.getByteCount() +
    ...
    setSize(width, height) {
        ...
        if (width !== w || height !== h) {
            ...
            mol_util_1.ValueCell.update(this.luminosityRenderable.values.uTexSizeInv, linear_algebra_1.Vec2.set(this.luminosityRenderable.values.uTexSizeInv.ref.value, 1 / width, 1 / height));
    ...
    update(colorOpaque, colorTransparent, emissive, depthOpaque, depthTransparent, props, camera, opaqueFogged) {
        let luminosityNeedsUpdate = false;
        if (this.luminosityRenderable.values.tColorOpaque.ref.value !== colorOpaque) {
            mol_util_1.ValueCell.update(this.luminosityRenderable.values.tColorOpaque, colorOpaque);
            luminosityNeedsUpdate = true;
        }
        if (this.luminosityRenderable.values.tColorTransparent.ref.value !== colorTransparent) {
            mol_util_1.ValueCell.update(this.luminosityRenderable.values.tColorTransparent, colorTransparent);
            luminosityNeedsUpdate = true;
        }
        ...
        if (this.luminosityRenderable.values.tDepthOpaque.ref.value !== depthOpaque) {
            mol_util_1.ValueCell.update(this.luminosityRenderable.values.tDepthOpaque, depthOpaque);
            luminosityNeedsUpdate = true;
        }
        if (this.luminosityRenderable.values.tDepthTransparent.ref.value !== depthTransparent) {
            mol_util_1.ValueCell.update(this.luminosityRenderable.values.tDepthTransparent, depthTransparent);
            luminosityNeedsUpdate = true;
        }
        ...
        mol_util_1.ValueCell.updateIfChanged(this.luminosityRenderable.values.uNear, camera.near);
        mol_util_1.ValueCell.updateIfChanged(this.luminosityRenderable.values.uFar, camera.far);
        mol_util_1.ValueCell.updateIfChanged(this.luminosityRenderable.values.uIsOrtho, camera.state.mode === 'orthographic' ? 1 : 0);
        mol_util_1.ValueCell.updateIfChanged(this.luminosityRenderable.values.uFogNear, camera.fogNear);
        mol_util_1.ValueCell.updateIfChanged(this.luminosityRenderable.values.uFogFar, camera.fogFar);
        mol_util_1.ValueCell.updateIfChanged(this.luminosityRenderable.values.uOpaqueFogged, opaqueFogged);
        if (luminosityNeedsUpdate) {
            this.luminosityRenderable.update();
        }
        mol_util_1.ValueCell.updateIfChanged(this.compositeRenderable.values.uBloomRadius, props.radius);
        mol_util_1.ValueCell.updateIfChanged(this.compositeRenderable.values.uBloomStrength, props.strength);
    }
...
const LuminositySchema = {
    ...util_1.QuadSchema,
    tColorOpaque: (0, schema_1.TextureSpec)('texture', 'rgba', 'ubyte', 'linear'),
    tColorTransparent: (0, schema_1.TextureSpec)('texture', 'rgba', 'ubyte', 'linear'),
    ...
    uIsOrtho: (0, schema_1.UniformSpec)('f'),
    uFogNear: (0, schema_1.UniformSpec)('f'),
    uFogFar: (0, schema_1.UniformSpec)('f'),
    uOpaqueFogged: (0, schema_1.UniformSpec)('b'),
...
function getLuminosityRenderable(ctx, colorOpaqueTexture, colorTransparentTexture, emissiveTexture, depthOpaqueTexture, depthTransparentTexture) {
    const width = colorOpaqueTexture.getWidth();
    const height = colorOpaqueTexture.getHeight();
    const values = {
        ...util_1.QuadValues,
        tColorOpaque: mol_util_1.ValueCell.create(colorOpaqueTexture),
        tColorTransparent: mol_util_1.ValueCell.create(colorTransparentTexture),
        tEmissive: mol_util_1.ValueCell.create(emissiveTexture),
        tDepthOpaque: mol_util_1.ValueCell.create(depthOpaqueTexture),
        tDepthTransparent: mol_util_1.ValueCell.create(depthTransparentTexture),
        ...
        uIsOrtho: mol_util_1.ValueCell.create(0),
        uFogNear: mol_util_1.ValueCell.create(10000),
        uFogFar: mol_util_1.ValueCell.create(10000),
        uOpaqueFogged: mol_util_1.ValueCell.create(true),
