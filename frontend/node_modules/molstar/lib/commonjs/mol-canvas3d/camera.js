...
const transition_1 = require("./camera/transition.js");
...
const euler_1 = require("../mol-math/linear-algebra/3d/euler.js");
const tmpClip = (0, vec4_1.Vec4)();
const tmpForward = (0, vec3_1.Vec3)();
const tmpRight = (0, vec3_1.Vec3)();
const tmpUp = (0, vec3_1.Vec3)();
const tmpBack = (0, vec3_1.Vec3)();
const tmpDelta = (0, vec3_1.Vec3)();
const tmpRotMat = mat4_1.Mat4.identity();
class Camera {
    get position() { return this.state.position; }
    set position(v) { vec3_1.Vec3.copy(this.state.position, v); }
    get up() { return this.state.up; }
    set up(v) { vec3_1.Vec3.copy(this.state.up, v); }
    get target() { return this.state.target; }
    set target(v) { vec3_1.Vec3.copy(this.state.target, v); }
    update() {
        const snapshot = this.state;
        if (snapshot.radiusMax === 0) {
            return false;
        }
        const distance = vec3_1.Vec3.distance(snapshot.position, snapshot.target);
        const minTargetDistance = this.minTargetDistance / this.scale;
        if (distance < minTargetDistance) {
            vec3_1.Vec3.sub(this.deltaDirection, snapshot.target, snapshot.position);
            vec3_1.Vec3.setMagnitude(this.deltaDirection, this.deltaDirection, minTargetDistance);
            vec3_1.Vec3.sub(snapshot.position, snapshot.target, this.deltaDirection);
        }
        const height = 2 * Math.tan(snapshot.fov / 2) * vec3_1.Vec3.distance(snapshot.position, snapshot.target) * this.scale;
        ...
        const changed = !mat4_1.Mat4.areEqual(this.projection, this.prevProjection, common_1.EPSILON) || !mat4_1.Mat4.areEqual(this.view, this.prevView, common_1.EPSILON);
        if (changed) {
            mat4_1.Mat4.mul(this.projectionView, this.projection, this.view);
            if (!mat4_1.Mat4.tryInvert(this.inverseProjectionView, this.projectionView)) {
                mat4_1.Mat4.copy(this.view, this.prevView);
                mat4_1.Mat4.copy(this.projection, this.prevProjection);
                mat4_1.Mat4.mul(this.projectionView, this.projection, this.view);
                return false;
            }
            mat4_1.Mat4.copy(this.prevView, this.view);
            mat4_1.Mat4.copy(this.prevProjection, this.projection);
            this.changed.next();
        }
        return changed;
    }
    setState(snapshot, durationMs, options) {
    ...
    getFocus(target, radius, up, dir, snapshot) {
        ...
        vec3_1.Vec3.sub(this.deltaDirection, (_a = snapshot === null || snapshot === void 0 ? void 0 : snapshot.target) !== null && _a !== void 0 ? _a : this.target, (_b = snapshot === null || snapshot === void 0 ? void 0 : snapshot.position) !== null && _b !== void 0 ? _b : this.position);
        if (dir)
            vec3_1.Vec3.matchDirection(this.deltaDirection, dir, this.deltaDirection);
        vec3_1.Vec3.setMagnitude(this.deltaDirection, this.deltaDirection, targetDistance);
        vec3_1.Vec3.sub(this.newPosition, target, this.deltaDirection);
        const state = Camera.copySnapshot(Camera.createDefaultSnapshot(), this.state);
        state.target = vec3_1.Vec3.clone(target);
        state.radius = r;
        state.position = vec3_1.Vec3.clone(this.newPosition);
        if (up)
            vec3_1.Vec3.matchDirection(state.up, up, state.up);
        return state;
    }
    getCenter(target, radius) {
        vec3_1.Vec3.sub(this.deltaDirection, this.target, this.position);
        vec3_1.Vec3.sub(this.newPosition, target, this.deltaDirection);
        const state = Camera.copySnapshot(Camera.createDefaultSnapshot(), this.state);
        state.target = vec3_1.Vec3.clone(target);
        state.position = vec3_1.Vec3.clone(this.newPosition);
    ...
    getInvariantFocus(target, radius, up, dir) {
        const r = Math.max(radius, 0.01);
        const targetDistance = this.getTargetDistance(r);
        vec3_1.Vec3.copy(this.deltaDirection, dir);
        vec3_1.Vec3.setMagnitude(this.deltaDirection, this.deltaDirection, targetDistance);
        vec3_1.Vec3.sub(this.newPosition, target, this.deltaDirection);
        const state = Camera.copySnapshot(Camera.createDefaultSnapshot(), this.state);
        state.target = vec3_1.Vec3.clone(target);
    ...
    getRay(out, x, y) {
        if (this.state.mode === 'orthographic') {
            vec3_1.Vec3.set(out.origin, x, y, 0);
            this.unproject(out.origin, out.origin);
            vec3_1.Vec3.normalize(out.direction, vec3_1.Vec3.sub(out.direction, this.target, this.position));
            vec3_1.Vec3.scaleAndAdd(out.origin, out.origin, out.direction, -this.near);
        }
        else {
            vec3_1.Vec3.copy(out.origin, this.state.position);
            vec3_1.Vec3.scale(out.origin, out.origin, this.scale);
            vec3_1.Vec3.set(out.direction, x, y, 0.5);
            this.unproject(out.direction, out.direction);
            vec3_1.Vec3.normalize(out.direction, vec3_1.Vec3.sub(out.direction, out.direction, out.origin));
        }
        return out;
    }
    ...
    getRotation(out) {
        ...
        vec3_1.Vec3.normalize(tmpForward, vec3_1.Vec3.sub(tmpForward, target, position));
        vec3_1.Vec3.normalize(tmpRight, vec3_1.Vec3.cross(tmpRight, tmpForward, up));
        vec3_1.Vec3.cross(tmpUp, tmpRight, tmpForward);
        mat4_1.Mat4.setIdentity(tmpRotMat);
        tmpRotMat[0] = tmpRight[0];
        tmpRotMat[1] = tmpRight[1];
        tmpRotMat[2] = tmpRight[2];
        tmpRotMat[4] = tmpUp[0];
        tmpRotMat[5] = tmpUp[1];
        tmpRotMat[6] = tmpUp[2];
        tmpRotMat[8] = -tmpForward[0];
        tmpRotMat[9] = -tmpForward[1];
        tmpRotMat[10] = -tmpForward[2];
        return euler_1.Euler.fromMat4(out, tmpRotMat, 'ZYX');
    }
    /** Set the camera rotation around its target. Expects 'ZYX' order. */
    setRotation(rotation, durationMs) {
        const snapshot = this.state;
        const distance = vec3_1.Vec3.distance(snapshot.position, snapshot.target);
        mat4_1.Mat4.fromEuler(tmpRotMat, rotation, 'ZYX');
        // back = R * (0,0,1) → column 2 of R
        vec3_1.Vec3.set(tmpBack, tmpRotMat[8], tmpRotMat[9], tmpRotMat[10]);
        // up = R * (0,1,0) → column 1 of R
        vec3_1.Vec3.set(tmpUp, tmpRotMat[4], tmpRotMat[5], tmpRotMat[6]);
        const state = Camera.copySnapshot(Camera.createDefaultSnapshot(), snapshot);
        vec3_1.Vec3.scaleAndAdd(state.position, snapshot.target, tmpBack, distance);
        vec3_1.Vec3.copy(state.up, tmpUp);
        this.setState(state, durationMs);
    }
    /** Translation of the camera target relative to world origin (0, 0, 0) */
    getTranslation(out) {
        return vec3_1.Vec3.copy(out, this.state.target);
    }
    /** Set the camera target to the given translation, moving position by the same delta so orientation/distance are preserved */
    setTranslation(translation, durationMs) {
        const snapshot = this.state;
        vec3_1.Vec3.sub(tmpDelta, translation, snapshot.target);
        const state = Camera.copySnapshot(Camera.createDefaultSnapshot(), snapshot);
        vec3_1.Vec3.add(state.position, snapshot.position, tmpDelta);
        vec3_1.Vec3.copy(state.target, translation);
        this.setState(state, durationMs);
    }
    constructor(state, viewport = util_1.Viewport.create(0, 0, 128, 128)) {
        this.view = mat4_1.Mat4.identity();
        this.projection = mat4_1.Mat4.identity();
        this.projectionView = mat4_1.Mat4.identity();
        this.inverseProjectionView = mat4_1.Mat4.identity();
        this.headRotation = mat4_1.Mat4.zero();
        this.viewEye = mat4_1.Mat4.zero();
        this.isAsymmetricProjection = false;
        ...
        this.prevProjection = mat4_1.Mat4.identity();
        this.prevView = mat4_1.Mat4.identity();
        this.deltaDirection = (0, vec3_1.Vec3)();
        this.newPosition = (0, vec3_1.Vec3)();
...
(function (Camera) {
    ...
    function createDefaultSnapshot() {
        return {
            mode: 'perspective',
            fov: Math.PI / 4,
            position: vec3_1.Vec3.create(0, 0, 100),
            up: vec3_1.Vec3.create(0, 1, 0),
            target: vec3_1.Vec3.create(0, 0, 0),
            radius: 0,
            radiusMax: 10,
            fog: 50,
            clipFar: true,
    ...
    function copySnapshot(out, source) {
        ...
        if (typeof source.position !== 'undefined')
            vec3_1.Vec3.copy(out.position, source.position);
        if (typeof source.up !== 'undefined')
            vec3_1.Vec3.copy(out.up, source.up);
        if (typeof source.target !== 'undefined')
            vec3_1.Vec3.copy(out.target, source.target);
    ...
    function areSnapshotsEqual(a, b) {
        return a.mode === b.mode
            ...
            && a.minNear === b.minNear
            && a.minFar === b.minFar
            && vec3_1.Vec3.exactEquals(a.position, b.position)
            && vec3_1.Vec3.exactEquals(a.up, b.up)
            && vec3_1.Vec3.exactEquals(a.target, b.target);
    }
    Camera.areSnapshotsEqual = areSnapshotsEqual;
})(Camera || (exports.Camera = Camera = {}));
const tmpPosition = (0, vec3_1.Vec3)();
const tmpTarget = (0, vec3_1.Vec3)();
function updateView(camera) {
    if (camera.scale === 1) {
        mat4_1.Mat4.lookAt(camera.view, camera.state.position, camera.state.target, camera.state.up);
    }
    else {
        vec3_1.Vec3.scale(tmpPosition, camera.state.position, camera.scale);
        vec3_1.Vec3.scale(tmpTarget, camera.state.target, camera.scale);
        mat4_1.Mat4.lookAt(camera.view, tmpPosition, tmpTarget, camera.state.up);
    }
}
function updateOrtho(camera) {
    ...
    mat4_1.Mat4.ortho(camera.projection, left, right, top, bottom, near, far);
    // build view matrix
    updateView(camera);
}
function updatePers(camera) {
    ...
    // build projection matrix
    mat4_1.Mat4.perspective(camera.projection, left, left + width, top, top - height, near, far);
    // build view matrix
    updateView(camera);
}
function updateClip(camera) {
    ...
    vec3_1.Vec3.scale(tmpTarget, camera.state.target, scale);
    vec3_1.Vec3.scale(tmpPosition, camera.state.position, scale);
    const cameraDistance = vec3_1.Vec3.distance(tmpPosition, tmpTarget);
