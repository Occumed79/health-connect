...
const simple_buffer_1 = require("../../common/simple-buffer.js");
const ascii_1 = require("../../common/ascii.js");
// ---------------------------------------------------------------------------
// Internal helpers
// ---------------------------------------------------------------------------
/**
 ...
 */
function detectLittleEndian(buf, fileSize) {
...
function parseSYMINF(payload) {
    ...
    const spgname = qStart >= 0 && qEnd > qStart
        ...
        : ((_a = parts[4]) !== null && _a !== void 0 ? _a : 'P 1');
 ...
 */
function parseCOL(payload, columnIndex) {
    var _a;
    ...
    const type = (_a = parts[n - 4]) !== null && _a !== void 0 ? _a : 'R';
 ...
 */
function parseCellPayload(payload) {
    var _a, _b, _c, _d, _e, _f;
    const p = payload.trim().split(/\s+/).map(Number);
    return [(_a = p[0]) !== null && _a !== void 0 ? _a : 0, (_b = p[1]) !== null && _b !== void 0 ? _b : 0, (_c = p[2]) !== null && _c !== void 0 ? _c : 0, (_d = p[3]) !== null && _d !== void 0 ? _d : 90, (_e = p[4]) !== null && _e !== void 0 ? _e : 90, (_f = p[5]) !== null && _f !== void 0 ? _f : 90];
}
// ---------------------------------------------------------------------------
// Main parser
// ---------------------------------------------------------------------------
 ...
 */
function parseMtz(buffer, name) {
    return mol_task_1.Task.create('Parse MTZ', async (ctx) => {
        var _a;
        try {
            if (buffer.length < 80) {
                return result_1.ReaderResult.error('MTZ file too small to be valid.');
            }
            // Validate magic bytes "MTZ "
            const magic = (0, ascii_1.asciiSlice)(buffer, 0, 4);
            if (magic.slice(0, 3) !== 'MTZ') {
                return result_1.ReaderResult.error('Not an MTZ file: magic bytes mismatch.');
            }
            const littleEndian = detectLittleEndian(buffer, buffer.length);
            const view = new DataView(buffer.buffer, buffer.byteOffset);
            const readInt32 = (byteOffset) => view.getInt32(byteOffset, littleEndian);
            const readFloat32 = (byteOffset) => view.getFloat32(byteOffset, littleEndian);
            ...
            if (headerBytePos <= 0 || headerBytePos >= buffer.length) {
                return result_1.ReaderResult.error(`MTZ header position out of range: word ${headerWordPos}`);
            }
            // ----------------------------------------------------------------
            ...
            while (pos + 80 <= buffer.length) {
                const record = (0, ascii_1.asciiSlice)(buffer, pos, pos + 80);
                pos += 80;
            ...
            for (const ds of datasetById.values())
                datasets.push(ds);
            datasets.sort((a, b) => a.id - b.id);
            if (nColumns === 0 || nReflections === 0) {
                return result_1.ReaderResult.error(`MTZ header did not contain valid NCOL record (nColumns=${nColumns}, nReflections=${nReflections}).`);
            }
            // ----------------------------------------------------------------
            ...
            if (dataByteEnd > buffer.length) {
                return result_1.ReaderResult.error(`MTZ file truncated: expected ${dataByteEnd} bytes for ${nReflections} × ${nColumns} reflections, got ${buffer.length}.`);
            }
            const data = new Float32Array(nValues);
            if (littleEndian === simple_buffer_1.SimpleBuffer.IsNativeEndianLittle) {
            ...
            return result_1.ReaderResult.success({ name, header, data });
        }
        catch (e) {
            return result_1.ReaderResult.error((_a = e === null || e === void 0 ? void 0 : e.message) !== null && _a !== void 0 ? _a : String(e));
        }
    });
}
