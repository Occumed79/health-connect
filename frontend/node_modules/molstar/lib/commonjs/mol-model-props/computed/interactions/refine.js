...
function waterBridgeRefiner(_structure, interactions) {
    const { contacts, bridges, unitsFeatures } = interactions;
    function atomKey(unitId, atomIndex) {
        return (0, hash_functions_1.cantorPairing)(unitId, atomIndex);
    }
    function featureMember(features, featureIndex) {
        return features.members[features.offsets[featureIndex]];
    }
    function addAtomPair(set, unitA, atomA, unitB, atomB) {
    ...
    function hasAtomPair(set, unitA, atomA, unitB, atomB) {
        var _a;
        return ((_a = set.get(atomKey(unitA, atomA))) === null || _a === void 0 ? void 0 : _a.has(atomKey(unitB, atomB))) === true;
    }
    function hasInfoPair(set, infoA, infoB) {
        ...
        for (let i = offsetsA[featureA], il = offsetsA[featureA + 1]; i < il; ++i) {
            ...
            for (let j = offsetsB[featureB], jl = offsetsB[featureB + 1]; j < jl; ++j) {
                const b = membersB[j];
                if (hasAtomPair(set, infoA.unit.id, a, infoB.unit.id, b))
                    return true;
            }
        }
        return false;
    }
    const bridgeLegs = new Map();
    for (const wb of bridges) {
        if (wb.props.type !== common_1.InteractionType.WaterBridge)
            continue;
    ...
    return {
        isApplicable: (type) => {
            return bridgeLegs.size > 0 && type === common_1.InteractionType.HydrogenBond;
        },
        handleInterContact: (index, infoA, infoB) => {
        ...
        handleIntraContact: (index, infoA, infoB) => {
            if (!intraContacts)
                return;
            if (hasInfoPair(bridgeLegs, infoA, infoB)) {
                intraContacts.edgeProps.flag[index] = common_1.InteractionFlag.Filtered;
            }
        },
    };
}
