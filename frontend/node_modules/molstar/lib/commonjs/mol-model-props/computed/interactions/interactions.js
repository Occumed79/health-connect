...
exports.InteractionsParams = exports.BridgeProviderParams = exports.ContactProviderParams = exports.Bridges = exports.Interactions = void 0;
...
const valence_model_1 = require("../valence-model.js");
const common_1 = require("./common.js");
const contacts_builder_1 = require("./contacts-builder.js");
const int_1 = require("../../../mol-data/int.js");
const contacts_1 = require("./contacts.js");
const halogen_bonds_1 = require("./halogen-bonds.js");
const hydrogen_bonds_1 = require("./hydrogen-bonds.js");
const water_bridges_1 = require("./water-bridges.js");
const charged_1 = require("./charged.js");
const hydrophobic_1 = require("./hydrophobic.js");
const set_1 = require("../../../mol-util/set.js");
const metal_1 = require("./metal.js");
const refine_1 = require("./refine.js");
const location_1 = require("../../../mol-model/location.js");
const centroid_helper_1 = require("../../../mol-math/geometry/centroid-helper.js");
...
var Bridges;
(function (Bridges) {
    function Location(data, bridgeIndex = 0) {
        return (0, location_1.DataLocation)('bridges', data, { bridgeIndex });
    }
    Bridges.Location = Location;
    function isLocation(x) {
        return !!x && x.kind === 'data-location' && x.tag === 'bridges';
    }
    Bridges.isLocation = isLocation;
    function Loci(data, elements) {
        return (0, loci_1.DataLoci)('bridges', data, elements, bs => getBoundingSphere(data, elements, bs), () => getLabel(data, elements));
    }
    Bridges.Loci = Loci;
    function isLoci(x) {
        return !!x && x.kind === 'data-loci' && x.tag === 'bridges';
    }
    Bridges.isLoci = isLoci;
    function getLabel(data, elements) {
        const e = elements[0];
        if (e === undefined)
            return '';
        const { structure, bridges, unitsFeatures } = data;
        const bridge = bridges[e.bridgeIndex];
        const uA = structure.unitMap.get(bridge.unitA);
        const fA = unitsFeatures.get(bridge.unitA);
        const uM = structure.unitMap.get(bridge.unitM);
        const fM = unitsFeatures.get(bridge.unitM);
        const uB = structure.unitMap.get(bridge.unitB);
        ...
        return [
            (0, common_1.interactionTypeLabel)(bridge.props.type),
            (0, label_1.bundleLabel)({ loci: [
                    structure_1.StructureElement.Loci(structure, [{ unit: uA, indices: int_1.OrderedSet.ofSingleton(fA.members[fA.offsets[bridge.indexA]]) }]),
                    structure_1.StructureElement.Loci(structure, [{ unit: uM, indices: int_1.OrderedSet.ofSingleton(fM.members[fM.offsets[bridge.indexMA]]) }]),
                    structure_1.StructureElement.Loci(structure, [{ unit: uB, indices: int_1.OrderedSet.ofSingleton(fB.members[fB.offsets[bridge.indexB]]) }]),
                ] }, options),
        ].join('</br>');
    }
    function getBoundingSphere(data, elements, boundingSphere) {
        return centroid_helper_1.CentroidHelper.fromPairProvider(elements.length * 2, (i, pA, pB) => {
            const bridge = data.bridges[elements[i >> 1].bridgeIndex];
            const uA = data.structure.unitMap.get(bridge.unitA);
            const fA = data.unitsFeatures.get(bridge.unitA);
            const uM = data.structure.unitMap.get(bridge.unitM);
            const fM = data.unitsFeatures.get(bridge.unitM);
            const uB = data.structure.unitMap.get(bridge.unitB);
...
})(Bridges || (exports.Bridges = Bridges = {}));
...
const BridgeProviders = {
    'water-bridges': water_bridges_1.WaterBridgesProvider,
};
function getBridgeProviderParams(defaultOn = []) {
    const params = Object.create(null);
    Object.keys(BridgeProviders).forEach(k => {
        params[k] = param_definition_1.ParamDefinition.MappedStatic(defaultOn.includes(k) ? 'on' : 'off', {
            on: param_definition_1.ParamDefinition.Group(BridgeProviders[k].params),
            off: param_definition_1.ParamDefinition.Group({})
        }, { cycle: true });
    });
    return params;
}
exports.BridgeProviderParams = getBridgeProviderParams([]);
exports.InteractionsParams = {
    providers: param_definition_1.ParamDefinition.Group(exports.ContactProviderParams, { isFlat: true }),
    bridges: param_definition_1.ParamDefinition.Group(exports.BridgeProviderParams, { isFlat: true }),
...
async function computeInteractions(ctx, structure, props, options) {
    ...
    (0, type_helpers_1.ObjectKeys)(BridgeProviders).forEach(k => {
        if (p.bridges[k].name === 'on')
            set_1.SetUtils.add(requiredFeatures, BridgeProviders[k].requiredFeatures);
    });
...
function findBridges(structure, unitsFeatures, props) {
    const bridges = [];
    (0, type_helpers_1.ObjectKeys)(BridgeProviders).forEach(k => {
        const { name, params } = props[k];
        if (name === 'on') {
            for (const b of BridgeProviders[k].find(structure, unitsFeatures, params))
                bridges.push(b);
        }
    });
    return bridges;
