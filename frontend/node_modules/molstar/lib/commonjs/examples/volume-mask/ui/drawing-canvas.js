...
function DrawingCanvas({ previewMask, onPolygonComplete }) {
    const canvasRef = (0, react_1.useRef)(null);
    const [inProgress, setInProgress] = (0, react_1.useState)([]);
    ...
    const paintRef = (0, react_1.useRef)(() => { });
    paintRef.current = () => {
        ...
        if (previewMask && inProgress.length === 0) {
            ...
            const scaled = previewMask.polygon.map(([x, y]) => [x * sx, y * sy]);
    ...
    (0, react_1.useEffect)(() => { paintRef.current(); }, [inProgress, mousePos, previewMask]);
    const getPos = (e) => {
        const r = canvasRef.current.getBoundingClientRect();
        return [e.clientX - r.left, e.clientY - r.top];
    };
    const completePoly = (0, react_1.useCallback)(() => {
        const canvas = canvasRef.current;
        onPolygonComplete === null || onPolygonComplete === void 0 ? void 0 : onPolygonComplete(inProgress, canvas.width, canvas.height);
        setInProgress([]);
        setMousePos(null);
    ...
    const handleMouseDown = (0, react_1.useCallback)((e) => {
        ...
        if (inProgress.length >= 3 && distPx(pos, inProgress[0]) < CLOSE_DIST_PX) {
            completePoly();
            return;
        }
    ...
    const handleDblClick = (0, react_1.useCallback)((e) => {
        e.stopPropagation();
        e.preventDefault();
    ...
    const handleKeyDown = (0, react_1.useCallback)((e) => {
        ...
        if ((e.key === 'Enter' || e.key === 'Return') && inProgress.length >= 3)
            completePoly();
    }, [inProgress, completePoly]);
    (0, react_1.useEffect)(() => {
    ...
    return ((0, jsx_runtime_1.jsx)("canvas", { ref: canvasRef, style: {
            ...
            zIndex: 10,
        }, onMouseDown: isPreviewOnly ? undefined : handleMouseDown, onMouseMove: isPreviewOnly ? undefined : handleMouseMove, onDoubleClick: isPreviewOnly ? undefined : handleDblClick }));
}
// ---- helpers ----------------------------------------------------------------
function distPx(a, b) {
...
function drawFilledPolygon(ctx, pts, color, alpha) {
    if (pts.length < 2)
        return;
    ctx.beginPath();
...
function drawPolygonOutline(ctx, pts, color, width, open = false) {
    if (pts.length < 2)
        return;
    ctx.beginPath();
    ctx.moveTo(pts[0][0], pts[0][1]);
...
function drawVertex(ctx, p, color, r = 4) {
    ctx.beginPath();
    ctx.arc(p[0], p[1], r, 0, Math.PI * 2);
    ctx.fillStyle = color;
    ctx.fill();
}
function drawFilledOutside(ctx, pts, w, h, color, alpha) {
    if (pts.length < 2)
        return;
    const r = parseInt(color.slice(1, 3), 16);
    const g = parseInt(color.slice(3, 5), 16);
