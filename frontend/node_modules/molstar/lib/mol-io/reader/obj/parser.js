...
import { parseInt, parseFloat } from '../common/text/number-parser.js';
...
function handleVertex(state) {
    ...
    let x = 0, y = 0, z = 0;
    if (readInlineToken(tokenizer))
        x = parseFloat(tokenizer.data, tokenizer.tokenStart, tokenizer.tokenEnd);
    if (readInlineToken(tokenizer))
        y = parseFloat(tokenizer.data, tokenizer.tokenStart, tokenizer.tokenEnd);
    if (readInlineToken(tokenizer))
        z = parseFloat(tokenizer.data, tokenizer.tokenStart, tokenizer.tokenEnd);
...
async function parseInternal(data, ctx) {
    ...
    const posArr = ChunkedArray.compact(state.positions);
    const normArr = ChunkedArray.compact(state.normals);
    const posIdxArr = ChunkedArray.compact(state.positionIndices);
    const normIdxArr = ChunkedArray.compact(state.normalIndices);
    const faceGroupsArr = ChunkedArray.compact(state.faceGroups);
    const vertexColorsArr = state.hasVertexColors
        ? ChunkedArray.compact(state.vertexColors)
        : new Float32Array(0);
    const result = {
        positions: posArr,
        normals: normArr,
