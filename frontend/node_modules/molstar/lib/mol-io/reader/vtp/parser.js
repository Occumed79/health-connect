 ...
 */
function toScalarArray(desc, values) {
    return {
        name: desc.name,
        type: desc.type,
...
function decodeFloat32(raw, count) {
...
function decodeInt64AsInt32(raw, count) {
    const dv = new DataView(raw.buffer, raw.byteOffset, raw.byteLength);
    const out = new Int32Array(count);
    for (let i = 0; i < count; i++)
        out[i] = dv.getUint32(i * 8, true);
    return out;
}
function decodeTyped(raw, type, count) {
    const dv = new DataView(raw.buffer, raw.byteOffset, raw.byteLength);
    switch (type) {
...
function decodePositions(raw, type, count) {
    if (type === 'Float32')
        return decodeFloat32(raw, count);
    return new Float32Array(decodeTyped(raw, type, count));
}
function decodeConnectivity(raw, type) {
...
function findBytesPosition(data, tag, from = 0) {
...
function extractAppendedInfo(data) {
    var _a, _b, _c;
    const tagPos = findBytesPosition(data, '<AppendedData');
    if (tagPos === -1)
        throw new Error('AppendedData section not found');
    let pos = tagPos;
    ...
    const encoding = ((_a = appendedAttrs.get('encoding')) !== null && _a !== void 0 ? _a : 'raw').toLowerCase();
    pos++; // skip '>'
    while (pos < data.length && (data[pos] === 0x20 || data[pos] === 0x09 || data[pos] === 0x0A || data[pos] === 0x0D))
        pos++;
    if (pos >= data.length || data[pos] !== 0x5F) {
        throw new Error(`Expected '_' before appended data, got 0x${(_c = (_b = data[pos]) === null || _b === void 0 ? void 0 : _b.toString(16)) !== null && _c !== void 0 ? _c : 'EOF'}`);
    }
 ...
 */
function findTagStart(src, tagName, from) {
    ...
    outer: for (let i = from, il = src.length - nl; i <= il; i++) {
        if (src.charCodeAt(i) !== 60)
            continue;
        for (let j = 1; j < nl; j++) {
            if (src.charCodeAt(i + j) !== needle.charCodeAt(j))
                continue outer;
        }
        const after = src.charCodeAt(i + nl);
        if (isWS(after) || after === 62 || after === 47)
            return i;
    }
    return -1;
}
 ...
 */
function readAttrs(src, pos) {
    const attrs = new Map();
    const len = src.length;
    while (pos < len) {
        while (pos < len && isWS(src.charCodeAt(pos)))
            pos++;
        const c = src.charCodeAt(pos);
        if (c === 62 || c === 47)
            break; // '>' or '/'
        const ns = pos;
        while (pos < len && src.charCodeAt(pos) !== 61 && !isWS(src.charCodeAt(pos)) && src.charCodeAt(pos) !== 62 && src.charCodeAt(pos) !== 47)
            pos++;
        ...
        if (pos < len && (src.charCodeAt(pos) === 34 || src.charCodeAt(pos) === 39)) {
            ...
            const vs = pos;
            while (pos < len && src.charCodeAt(pos) !== q)
                pos++;
            if (name)
                attrs.set(name, src.slice(vs, pos));
            if (pos < len)
                pos++;
        }
    }
...
// --- XML header parsing ---
function parseDataArraysInSection(header, sectionName) {
    var _a, _b, _c;
    const sStart = findTagStart(header, sectionName, 0);
    if (sStart === -1)
        return [];
    let pos = sStart + 1 + sectionName.length;
    while (pos < header.length && header.charCodeAt(pos) !== 62)
        pos++;
    pos++; // skip '>'
    ...
    while (pos < sEnd) {
        ...
        pos = nextPos;
        const fmt = ((_a = attrs.get('format')) !== null && _a !== void 0 ? _a : 'binary');
        const offsetStr = attrs.get('offset');
        const b64 = fmt === 'binary' ? content.replace(/\s/g, '') : '';
        const asciiText = fmt === 'ascii' ? content : undefined;
        if (fmt === 'appended' && offsetStr === undefined)
            continue;
        if (fmt === 'binary' && b64.length === 0)
            continue;
        const rangeMinStr = attrs.get('RangeMin');
        const rangeMaxStr = attrs.get('RangeMax');
        results.push({
            name: (_b = attrs.get('Name')) !== null && _b !== void 0 ? _b : '',
            type: toVtkDataType(attrs.get('type')),
            numberOfComponents: parseInt((_c = attrs.get('NumberOfComponents')) !== null && _c !== void 0 ? _c : '1', 10),
            format: fmt,
...
function parseHeader(header) {
    ...
    if (vtkStart === -1)
        throw new Error('Cannot find VTKFile element in VTP header');
    const { attrs: vtkAttrs } = readAttrs(header, vtkStart + '<VTKFile'.length);
    const pieceStart = findTagStart(header, 'Piece', 0);
    if (pieceStart === -1)
        throw new Error('Cannot find Piece element in VTP header');
    ...
    return {
        ...
        nVerts: parseInt((_a = pieceAttrs.get('NumberOfVerts')) !== null && _a !== void 0 ? _a : '0', 10),
        nLines: parseInt((_b = pieceAttrs.get('NumberOfLines')) !== null && _b !== void 0 ? _b : '0', 10),
        nStrips: parseInt((_c = pieceAttrs.get('NumberOfStrips')) !== null && _c !== void 0 ? _c : '0', 10),
        byteOrder: (_d = vtkAttrs.get('byte_order')) !== null && _d !== void 0 ? _d : 'LittleEndian',
        compressor: (_e = vtkAttrs.get('compressor')) !== null && _e !== void 0 ? _e : '',
 ...
 */
async function decompressVtkBlockB64Bytes(ctx, rawData, rawByteStart, byteOffset, headerType = 'UInt32', hasCompressor = true) {
    ...
    for (let i = 0; i < nblocks; i++) {
        ...
        const inflated = await inflate(ctx, new Uint8Array(compData.buffer, compData.byteOffset + compOff, csize));
...
async function decompressVtkBlockB64Str(ctx, src, charOffset, headerType = 'UInt32', hasCompressor = true) {
    const hdrItemBytes = headerType === 'UInt64' ? 8 : 4;
    if (!hasCompressor) {
...
async function decompressVtkBlockRaw(ctx, rawData, rawByteStart, byteOffset, headerType = 'UInt32', hasCompressor = true) {
    const hdrItemBytes = headerType === 'UInt64' ? 8 : 4;
    ...
    for (let i = 0; i < nblocks; i++) {
        const csize = compSizes[i];
        const blockView = new Uint8Array(rawData.buffer, rawData.byteOffset + compOff, csize);
...
async function decompressBlock(ctx, info, desc, headerType, hasCompressor) {
    if (desc.inlineBase64) {
        return decompressVtkBlockB64Str(ctx, desc.inlineBase64, 0, headerType, hasCompressor);
    }
    if (!info)
        throw new Error(`No data source for array "${desc.name}"`);
 ...
 */
function parseAsciiNumbers(text, out, count, asInt) {
    let pos = 0, n = 0;
    const len = text.length;
    while (pos < len && n < count) {
        while (pos < len && isWS(text.charCodeAt(pos)))
            pos++;
        if (pos >= len)
            break;
        const start = pos;
        while (pos < len && !isWS(text.charCodeAt(pos)))
            pos++;
        if (pos > start)
            out[n++] = asInt ? fastParseInt(text, start, pos) : fastParseFloat(text, start, pos);
    }
}
function parseAsciiFloat32(text, count) {
    const out = new Float32Array(count);
    parseAsciiNumbers(text, out, count, false);
    return out;
}
function parseAsciiFloat64(text, count) {
    const out = new Float64Array(count);
    parseAsciiNumbers(text, out, count, false);
    return out;
}
function parseAsciiInts(text, count) {
    const out = new Int32Array(count);
...
function parseAsciiIntsAll(text) {
    let count = 0, pos = 0;
    const len = text.length;
    while (pos < len) {
        while (pos < len && isWS(text.charCodeAt(pos)))
 ...
 */
function buildTriangles(rawConn, offsets, nCells) {
    let nTris = 0;
...
async function parseInternal(data, ctx) {
    ...
    const headerText = new TextDecoder('ascii').decode(isInline ? data : data.subarray(0, appendedTagPos));
    ...
    // Points positions
    const pointsDesc = (_a = hdr.pointsArrays.find(d => d.name === 'Points')) !== null && _a !== void 0 ? _a : hdr.pointsArrays[0];
    if (!pointsDesc)
        throw new Error('VTP file missing Points DataArray');
    ctx.update({ message: 'Decoding positions...', current: 10, max: 100 });
    let positions;
    if (pointsDesc.format === 'ascii') {
        positions = parseAsciiFloat32((_b = pointsDesc.asciiText) !== null && _b !== void 0 ? _b : '', hdr.nPoints * 3);
    }
    else {
    ...
    if (!connDesc || !offsetsDesc)
        throw new Error('VTP file missing Polys connectivity/offsets DataArrays');
    ...
    if (connDesc.format === 'ascii') {
        ...
        rawConn = parseAsciiIntsAll((_c = connDesc.asciiText) !== null && _c !== void 0 ? _c : '');
        rawOffsets = parseAsciiInts((_d = offsetsDesc.asciiText) !== null && _d !== void 0 ? _d : '', hdr.nCells);
    }
    else {
    ...
    for (let i = 0; i < hdr.pointDataArrays.length; i++) {
        ...
        if (desc.format === 'ascii') {
            values = parseAsciiFloat64((_e = desc.asciiText) !== null && _e !== void 0 ? _e : '', totalCount);
        }
        else {
    ...
    for (let i = 0; i < hdr.cellDataArrays.length; i++) {
        ...
        let values;
        if (desc.format === 'ascii') {
            values = parseAsciiFloat64((_f = desc.asciiText) !== null && _f !== void 0 ? _f : '', totalCount);
        }
        else {
            const raw = await decompressBlock(ctx, info, desc, headerType, hasCompressor);
            values = decodeTyped(raw, desc.type, totalCount);
