...
import { SimpleBuffer } from '../../common/simple-buffer.js';
import { asciiSlice } from '../../common/ascii.js';
// ---------------------------------------------------------------------------
// Internal helpers
// ---------------------------------------------------------------------------
/**
 ...
 */
export function parseMtz(buffer, name) {
    return Task.create('Parse MTZ', async (ctx) => {
        var _a;
        try {
            if (buffer.length < 80) {
                return Result.error('MTZ file too small to be valid.');
            }
            // Validate magic bytes "MTZ "
            const magic = asciiSlice(buffer, 0, 4);
            if (magic.slice(0, 3) !== 'MTZ') {
                return Result.error('Not an MTZ file: magic bytes mismatch.');
            }
            ...
            if (headerBytePos <= 0 || headerBytePos >= buffer.length) {
                return Result.error(`MTZ header position out of range: word ${headerWordPos}`);
            }
            // ----------------------------------------------------------------
            ...
            if (nColumns === 0 || nReflections === 0) {
                return Result.error(`MTZ header did not contain valid NCOL record (nColumns=${nColumns}, nReflections=${nReflections}).`);
            }
            // ----------------------------------------------------------------
            ...
            if (dataByteEnd > buffer.length) {
                return Result.error(`MTZ file truncated: expected ${dataByteEnd} bytes for ${nReflections} × ${nColumns} reflections, got ${buffer.length}.`);
            }
            ...
            return Result.error((_a = e === null || e === void 0 ? void 0 : e.message) !== null && _a !== void 0 ? _a : String(e));
        }
    });
}
