    ...
    if (info.ext === 'bcif') {
        try {
            ...
            if (bcifHasCategory(file, '_volume_data_3d_info')) {
                if (bcifHasCategory(file, '_volume_data_3d'))
                    return 'dscif';
                if (bcifHasCategory(file, '_segmentation_data_3d'))
                    return 'segcif';
            }
        ...
        if (str.includes('_refln.pdbx_FWT') || str.includes('_refln.pdbx_DELFWT'))
            return 'sfcif';
...
function bcifHasColumn(file, categoryName, columnName) {
    for (const block of file.dataBlocks) {
        for (const category of block.categories) {
            if (category.name === categoryName) {
