...
import { MoleculeType, getMoleculeType } from '../../mol-model/structure/model/types.js';
    ...
    return Task.create('Parse GRO', async (ctx) => {
        ...
        for (let i = 0, il = gro.structures.length; i < il; ++i) {
            ...
            if (m.frameCount === 1) {
                ...
                if (bx !== 0 && by !== 0 && bz !== 0) {
                    const symmetry = ModelSymmetry.fromCell(Vec3.create(bx * 10, by * 10, bz * 10), Vec3.create(Math.PI / 2, Math.PI / 2, Math.PI / 2));
