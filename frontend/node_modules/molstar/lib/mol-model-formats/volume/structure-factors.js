...
import { SpacegroupCell, Box3D, Spacegroup } from '../../mol-math/geometry.js';
...
import { computeElectronDensityFromReflections } from './shared/reflections.js';
export function volumeFromStructureFactors(source, params) {
    return Task.create('Compute Structure Factor Map', async (ctx) => {
        var _a;
        const mapType = (_a = params === null || params === void 0 ? void 0 : params.mapType) !== null && _a !== void 0 ? _a : '2fo-fc';
        // 1. Parse cell parameters
        const { cell, symmetry, refln } = source;
        const spaceGroupId = symmetry.Int_Tables_number.value(0) ||
            symmetry['space_group_name_H-M'].value(0) ||
            'P 1';
        const cellSize = Vec3.create(cell.length_a.value(0), cell.length_b.value(0), cell.length_c.value(0));
        const cellAngles = Vec3.create(degToRad(cell.angle_alpha.value(0)), degToRad(cell.angle_beta.value(0)), degToRad(cell.angle_gamma.value(0)));
        ...
        if (count === 0)
            throw new Error('No reflection data found in the _refln category.');
        ...
        const { density, N0, N1, N2 } = await computeElectronDensityFromReflections(hArr, kArr, lArr, ampCol, phiCol, sg, ctx);
...
(function (SfFormat) {
    function is(x) {
        return (x === null || x === void 0 ? void 0 : x.kind) === 'sf';
    }
    SfFormat.is = is;
    function create(db) {
        return { kind: 'sf', name: db._name, data: db };
    }
    SfFormat.create = create;
})(SfFormat || (SfFormat = {}));
