...
function vtkMouseCameraUnicamRotateManipulator(publicAPI, model) {
	...
	const rotate = (interactor, position) => {
		...
		if (Math.abs(dot$1) > 1e-4) {
			...
			normalize(rightV);
			if (model.useWorldUpVec) {
				if (dot(upVec, atV) > .99 && rDist < 0) rDist = 0;
				if (dot(upVec, atV) < -.99 && rDist > 0) rDist = 0;
			}
