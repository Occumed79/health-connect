...
//#region Sources/Common/DataModel/CellLinks/index.js
var InitLink = {
	ncells: 0,
	cells: null
};
function resize(model, sz) {
	let newSize = sz;
	if (sz >= model.array.length) newSize += model.array.length;
...
function vtkCellLinks(publicAPI, model) {
	...
	*/
	publicAPI.buildLinks = (data) => {
		...
		if (data.isA("vtkPolyData")) {
			...
			for (let cellId = 0; cellId < numCells; cellId++) Cell_default.newInstance().getPointsIds().forEach((cellPointId) => {
				publicAPI.incrementLinkCount(cellPointId);
			});
			publicAPI.allocateLinks(numPts);
			model.maxId = numPts - 1;
	...
	*/
	publicAPI.allocate = (numLinks, ext = 1e3) => {
		...
		for (let i = 0; i < numLinks; ++i) array[i] = {
			ncells: 0,
			cells: null
		};
		model.array = array;
	...
	*/
	...
	publicAPI.allocateLinks = (n) => {
		for (let i = 0; i < n; ++i) model.array[i].cells = new Array(model.array[i].ncells);
	};
	/**
...
var newInstance = macros_default.newInstance(extend, "vtkCellLinks");
var CellLinks_default = {
	newInstance,
	extend
};
//#endregion
export { InitLink, CellLinks_default as default, extend, newInstance };

//# sourceMappingURL=CellLinks.js.map