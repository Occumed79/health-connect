import { macros_exports, newInstance as newInstance$1, obj } from "../../macros.js";
...
import UniformBuffer_default from "./UniformBuffer.js";
import { addClipPlaneEntries, getClipPlaneShaderChecks, getClippingPlaneEquationsInCoords } from "./Helpers/ClippingPlanes.js";
import { registerOverride } from "./ViewNodeFactory.js";
import { TextureChannelMode, TextureSlot } from "./ImageMapper/Constants.js";
import { computeFnToString, getLUTRowCenterExpression, getTextureChannelMode, getUseLabelOutline, textureSamplerMatches } from "./Helpers/ImageSampling.js";
import { mat4, vec4 } from "gl-matrix";
//#region Sources/Rendering/WebGPU/ImageMapper/index.js
...
var tmpTranslate3 = new Float64Array(3);
var tmpScale3 = new Float64Array(3);
...
function vtkWebGPUImageMapper(publicAPI, model) {
	...
	publicAPI.ensureTextureSampler = (textureView, options) => {
		if (textureView && !textureSamplerMatches(textureView, options)) textureView.addSampler(model.device, options);
	};
	publicAPI.computeImageState = () => {
		const actorProperty = model.WebGPUImageSlice.getRenderable().getProperty();
		const numberOfComponents = publicAPI.getTextureViews()[TextureSlot.IMAGE]?.getTexture().getNumberOfComponents() ?? 1;
		const independentComponents = actorProperty.getIndependentComponents();
		return {
			actorProperty,
			numberOfComponents,
			independentComponents,
			numberOfIComponents: independentComponents ? numberOfComponents : 1,
			useLabelOutline: getUseLabelOutline(actorProperty, independentComponents, numberOfComponents),
			textureChannelMode: getTextureChannelMode(independentComponents, numberOfComponents)
		};
	};
	publicAPI.getImageState = () => model.imageState ?? publicAPI.computeImageState();
	publicAPI.buildPass = (prepass) => {
		if (prepass) {
			...
			if (model.renderable.getSliceAtFocalPoint()) model.renderable.setSliceFromCamera(ren.getActiveCamera());
		}
	};
	...
	publicAPI.computePipelineHash = () => {
		...
		if (imageState.useLabelOutline) model.pipelineHash += "outline";
	...
	publicAPI.updateUBO = () => {
		...
		if (publicAPI.getMTime() > utime || model.renderable.getMTime() > utime || actor.getProperty().getMTime() > utime || clippingPlanesMTime > utime) {
			...
			tmpScale3[0] = 1 / dims[0];
			tmpScale3[1] = 1 / dims[1];
			tmpScale3[2] = 1 / dims[2];
			...
			if (ijkMode !== model.renderable.getSlicingMode()) nSlice = model.renderable.getSliceAtPosition(nSlice);
			let axis0 = 2;
			let axis1 = 0;
			let axis2 = 1;
			...
			ptsArray1[axis1] = ext[axis1 * 2] - .5;
			ptsArray1[axis2] = ext[axis2 * 2] - .5;
			ptsArray1[3] = 1;
			vec4.transformMat4(ptsArray1, ptsArray1, tmp3Mat4);
			model.UBO.setArray("Origin", ptsArray1);
			ptsArray2[axis0] = nSlice;
			ptsArray2[axis1] = ext[axis1 * 2 + 1] + .5;
			ptsArray2[axis2] = ext[axis2 * 2] - .5;
			ptsArray2[3] = 1;
			...
			ptsArray2[axis0] = nSlice;
			ptsArray2[axis1] = ext[axis1 * 2] - .5;
			ptsArray2[axis2] = ext[axis2 * 2 + 1] + .5;
			ptsArray2[3] = 1;
			vec4.transformMat4(ptsArray2, ptsArray2, tmp3Mat4);
			vec4.subtract(ptsArray2, ptsArray2, ptsArray1);
			ptsArray2[3] = 1;
			...
			componentWeight[2] = 1;
			componentWeight[3] = 1;
			const tScale = model.textureViews[0].getTexture().getScale();
			const { numberOfComponents: numComp, independentComponents: iComps } = publicAPI.getImageState();
			...
			if (numClipPlanes > 0) {
				...
				getClippingPlaneEquationsInCoords(model.renderable, tmp2Mat4, model.clipPlanes);
				for (let i = 0; i < numClipPlanes; i++) model.UBO.setArray(`ClipPlane${i}`, model.clipPlanes[i]);
			}
	...
	publicAPI.updateLUTImage = () => {
		...
		if (model.colorTextureString !== cfunToString) {
			...
			if (!model.colorLUTArray || model.colorLUTArray.length !== colorSize) model.colorLUTArray = new Uint8ClampedArray(colorSize);
			...
			if (cfun) {
				...
				for (let c = 0; c < numIComps; c++) {
					...
					for (let i = 0; i < model.rowLength; i++) {
						...
						for (let j = 0; j < 4; j++) colorArray[idx + model.rowLength * 4 + j] = colorArray[idx + j];
					}
				}
			} else for (let i = 0; i < model.rowLength; ++i) {
				const grey = 255 * i / (model.rowLength - 1);
				colorArray[i * 4] = grey;
				colorArray[i * 4 + 1] = grey;
				colorArray[i * 4 + 2] = grey;
				colorArray[i * 4 + 3] = 255;
				for (let j = 0; j < 4; j++) colorArray[i * 4 + model.rowLength * 4 + j] = colorArray[i * 4 + j];
			}
			{
				const treq = {
				...
				const tview = model.device.getTextureManager().getTexture(treq).createView("tfunTexture");
	...
	publicAPI.updateOpacityLUTImage = () => {
		...
		if (model.opacityTextureString !== pwfunToString) {
			...
			if (!model.opacityLUTArray || model.opacityLUTArray.length !== opacitySize) model.opacityLUTArray = new Float32Array(opacitySize);
			...
			for (let c = 0; c < numIComps; c++) {
				...
				if (!pwfun) {
					...
					opacityArray.fill(1, offset, offset + model.rowLength * 2);
					continue;
				}
				const pwfRange = pwfun.getRange();
			...
			const tview = model.device.getTextureManager().getTexture(treq).createView("ofunTexture");
			publicAPI.ensureTextureSampler(tview, {
				minFilter: "linear",
				magFilter: "linear"
			});
	...
	publicAPI.updateLabelOutlineThicknessTexture = () => {
		const labelOutlineThicknessArray = model.WebGPUImageSlice.getRenderable().getProperty().getLabelOutlineThicknessByReference();
		...
		if (model.labelOutlineThicknessString !== outlineHash) {
			...
			for (let i = 0; i < lWidth; ++i) outlineArray[i] = typeof labelOutlineThicknessArray[i] !== "undefined" ? labelOutlineThicknessArray[i] : labelOutlineThicknessArray[0];
			const treq = {
			...
			const tview = model.device.getTextureManager().getTexture(treq).createView("labelOutlineTexture");
			publicAPI.ensureTextureSampler(tview, {
				minFilter: "nearest",
				magFilter: "nearest"
			});
	...
	publicAPI.updateLabelOutlineOpacityTexture = () => {
		const labelOutlineOpacity = model.WebGPUImageSlice.getRenderable().getProperty().getLabelOutlineOpacity();
		const values = Array.isArray(labelOutlineOpacity) ? labelOutlineOpacity : [labelOutlineOpacity];
		const lWidth = Math.max(1, model.renderable.getLabelOutlineTextureWidth());
		...
		if (model.labelOutlineOpacityString !== outlineHash) {
			...
			for (let i = 0; i < lWidth; ++i) outlineArray[i] = values[i] ?? values[0];
			const treq = {
				nativeArray: outlineArray,
				width: lWidth,
				height: 1,
			...
			const tview = model.device.getTextureManager().getTexture(treq).createView("labelOutlineOpacityTexture");
			publicAPI.ensureTextureSampler(tview, {
				minFilter: "nearest",
				magFilter: "nearest"
			});
	...
	publicAPI.updateBuffers = () => {
		...
		if (!tViews[TextureSlot.IMAGE] || tViews[TextureSlot.IMAGE].getTexture() !== newTex) {
		...
		if (model.imageState.useLabelOutline) {
			publicAPI.updateLabelOutlineThicknessTexture();
			publicAPI.updateLabelOutlineOpacityTexture();
		} else model.textureViews.length = Math.min(model.textureViews.length, TextureSlot.OPACITY_LUT + 1);
		publicAPI.updateUBO();
		const iType = model.WebGPUImageSlice.getRenderable().getProperty().getInterpolationType() === InterpolationType.NEAREST ? "nearest" : "linear";
	...
	publicAPI.replaceShaderPosition = (hash, pipeline, vertexInput) => {
		...
		if (model.dimensions === 2) lines.push("var tcoord : vec2<f32> = (mapperUBO.SCTCMatrix * pos).xy;");
		else lines.push("var tcoord : vec3<f32> = (mapperUBO.SCTCMatrix * pos).xyz;");
		lines.push("output.vertexSC = pos;", "pos = rendererUBO.SCPCMatrix * pos;", "pos.z = clamp(pos.z - 0.000016 * mapperUBO.CoincidentOffset * pos.w, 0.0, pos.w);", "output.tcoordVS = tcoord;", "output.Position = pos;");
		code = ShaderCache_default.substitute(code, "//VTK::Position::Impl", lines).result;
		vDesc.setCode(code);
	};
	sr.set("replaceShaderPosition", publicAPI.replaceShaderPosition);
	publicAPI.replaceShaderTCoord = (hash, pipeline, vertexInput) => {
		const vDesc = pipeline.getShaderDescription("vertex");
		if (model.dimensions === 2) vDesc.addOutput("vec2<f32>", "tcoordVS");
		else vDesc.addOutput("vec3<f32>", "tcoordVS");
	};
	...
	publicAPI.replaceShaderImage = (hash, pipeline, vertexInput) => {
		...
		const imageState = publicAPI.getImageState();
		code = ShaderCache_default.substitute(code, "//VTK::Image::Sample", [
		...
		switch (imageState.textureChannelMode) {
			case TextureChannelMode.SINGLE:
				if (imageState.useLabelOutline) {
					const outlineLines = model.dimensions === 3 ? [
				...
				} else code = ShaderCache_default.substitute(code, "//VTK::Image::Sample", [
	...
	publicAPI.replaceShaderClip = (hash, pipeline, vertexInput) => {
		...
		code = ShaderCache_default.substitute(code, "//VTK::Clip::Impl", [...clipPlaneChecks, "//VTK::Clip::Impl"]).result;
		fDesc.setCode(code);
	};
	...
	publicAPI.replaceShaderCoincidentOffset = (hash, pipeline, vertexInput) => {
		...
		let code = fDesc.getCode();
		code = ShaderCache_default.substitute(code, "//VTK::Position::Impl", [
	...
	sr.set("replaceShaderCoincidentOffset", publicAPI.replaceShaderCoincidentOffset);
	publicAPI.replaceShaderSelect = (hash, pipeline, vertexInput) => {
		const fDesc = pipeline.getShaderDescription("fragment");
		if (!fDesc) return;
		let code = fDesc.getCode();
		code = ShaderCache_default.substitute(code, "//VTK::Select::Impl", ["  var compositeID: u32 = 0u;", "  var attributeID: u32 = 0u;"]).result;
		fDesc.setCode(code);
	};
...
function extend(publicAPI, model, initialValues = {}) {
	...
	FullScreenQuad_default.extend(publicAPI, model, initialValues);
	publicAPI.setFragmentShaderTemplate(imgFragTemplate);
	model.UBO = UniformBuffer_default.newInstance({ label: "mapperUBO" });
	...
	model.lutBuildTime = {};
	obj(model.lutBuildTime, { mtime: 0 });
	...
	model.clipPlanes = Array.from({ length: 6 }, () => [
		0,
		0,
		0,
		0
