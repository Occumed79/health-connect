...
//#region Sources/Rendering/OpenGL/SurfaceLIC/LineIntegralConvolution2D/pingpong.js
...
function allocateLICBuffer(openGLRenderWindow, size) {
	return allocateBuffer(openGLRenderWindow, size, Texture_default.Filter.NEAREST, Texture_default.Wrap.CLAMP_TO_EDGE);
}
function allocateNoiseBuffer(openGLRenderWindow, size) {
	return allocateBuffer(openGLRenderWindow, size, Texture_default.Filter.NEAREST, Texture_default.Wrap.CLAMP_TO_EDGE);
}
function allocateVectorBuffer(openGLRenderWindow, size) {
	return allocateBuffer(openGLRenderWindow, size, Texture_default.Filter.LINEAR, Texture_default.Wrap.CLAMP_TO_EDGE);
}
function vtkLICPingPongBufferManager(publicAPI, model) {
	...
	publicAPI.clearBuffers = (clearEETex = false) => {
		...
		fb.removeColorBuffer(2);
		fb.removeColorBuffer(3);
		if (clearEETex) fb.removeColorBuffer(4);
	...
	publicAPI.activateVectorTextures = () => {
		if (model.imageVectorTexture) model.imageVectorTexture.activate();
		else model.vectorTexture.activate();
		if (model.maskVectorTexture) model.maskVectorTexture.activate();
	};
	publicAPI.deactivateVectorTextures = () => {
		if (model.imageVectorTexture) model.imageVectorTexture.deactivate();
		else model.vectorTexture.deactivate();
		if (model.maskVectorTexture) model.maskVectorTexture.deactivate();
	};
	publicAPI.activateNoiseTexture = (licPassNum = 0) => {
		switch (licPassNum) {
			...
			case 1:
				model.eeTexture.activate();
				break;
			default: console.error("Wrong LIC pass number");
		}
	};
	...
	publicAPI.attachEEBuffer = () => {
		model.textures[model.readIndex][0].activate();
	...
	publicAPI.detachEEBuffer = () => {
		...
		model.framebuffer.removeColorBuffer(0);
		gl.drawBuffers([gl.NONE]);
		model.textures[model.readIndex][0].deactivate();
	};
	publicAPI.detachBuffers = () => {
		...
		if (model.eeTexture) model.eeTexture.deactivate();
		if (model.noiseTexture) model.noiseTexture.deactivate();
	};
...
function extend(publicAPI, model, initialValues = {}) {
	...
	macros_default.get(publicAPI, model, ["readIndex"]);
...
var newInstance = macros_default.newInstance(extend, "vtkLICPingPongBufferManager");
var pingpong_default = {
	newInstance,
	extend
};
//#endregion
export { pingpong_default as default, extend, newInstance };
