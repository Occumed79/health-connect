import { chain, macros_exports, newInstance as newInstance$1, obj, setGet, vtkErrorMacro as vtkErrorMacro$1 } from "../../macros.js";
...
import Constants_default from "../Core/ImageMapper/Constants.js";
...
//#region Sources/Rendering/OpenGL/ImageMapper/index.js
var { vtkErrorMacro } = macros_exports;
var { SlicingMode } = Constants_default;
function splitStringOnEnter(inputString) {
	const lines = inputString.split("\n");
	const trimmedLines = [];
	for (let i = 0; i < lines.length; ++i) {
		const trimmedLine = lines[i].trim();
		if (trimmedLine.length > 0) trimmedLines.push(trimmedLine);
	}
	return trimmedLines;
}
function vtkOpenGLImageMapper(publicAPI, model) {
	...
	publicAPI.buildPass = (prepass) => {
		if (prepass) {
			...
			if (model.renderable.isA("vtkImageMapper") && model.renderable.getSliceAtFocalPoint()) model.renderable.setSliceFromCamera(ren.getActiveCamera());
		}
	};
	...
	publicAPI.replaceShaderValues = (shaders, ren, actor) => {
		...
		if (actor.getProperty().getUseLabelOutline()) tcoordDec = tcoordDec.concat(["uniform sampler2D labelOutlineTexture1;", "uniform sampler2D labelOutlineOpacityTexture1;"]);
		if (iComps) {
			for (let comp = 1; comp < tNumComp; comp++) tcoordDec = tcoordDec.concat([
		...
		FSSource = ShaderProgram_default.substitute(FSSource, "//VTK::TCoord::Dec", tcoordDec).result;
		if (actor.getProperty().getUseLabelOutline() === true) {
			FSSource = ShaderProgram_default.substitute(FSSource, "//VTK::LabelOutline::Dec", [
			...
			FSSource = ShaderProgram_default.substitute(FSSource, "//VTK::ImageLabelOutlineOn", "#define vtkImageLabelOutlineOn").result;
			FSSource = ShaderProgram_default.substitute(FSSource, "//VTK::LabelOutlineHelperFunction", [
		...
		if (iComps) {
			...
			for (let comp = 0; comp < tNumComp; comp++) tcoordImpl = tcoordImpl.concat([`vec3 tcolor${comp} = mix${comp} * texture2D(colorTexture1, vec2(tvalue.${rgba[comp]} * cscale${comp} + cshift${comp}, height${comp})).rgb;`, `float compWeight${comp} = mix${comp} * texture2D(pwfTexture1, vec2(tvalue.${rgba[comp]} * pwfscale${comp} + pwfshift${comp}, height${comp})).r;`]);
			switch (tNumComp) {
				case 1:
			...
			FSSource = ShaderProgram_default.substitute(FSSource, "//VTK::TCoord::Impl", tcoordImpl).result;
		} else switch (tNumComp) {
			case 1:
				FSSource = ShaderProgram_default.substitute(FSSource, "//VTK::TCoord::Impl", [...splitStringOnEnter(`
                #ifdef vtkImageLabelOutlineOn
	...
	publicAPI.replaceShaderClip = (shaders, ren, actor) => {
		...
		if (model.renderable.getNumberOfClippingPlanes()) {
			...
			FSSource = ShaderProgram_default.substitute(FSSource, "//VTK::Clip::Dec", ["uniform int numClipPlanes;", "varying float clipDistancesVSOutput[6];"]).result;
	...
	publicAPI.setMapperShaderParameters = (cellBO, ren, actor) => {
		if (cellBO.getCABO().getElementCount() && (model.VBOBuildTime > cellBO.getAttributeUpdateTime().getMTime() || cellBO.getShaderSourceTime().getMTime() > cellBO.getAttributeUpdateTime().getMTime())) {
			if (cellBO.getProgram().isAttributeUsed("vertexMC")) {
				if (!cellBO.getVAO().addAttributeArray(cellBO.getProgram(), cellBO.getCABO(), "vertexMC", cellBO.getCABO().getVertexOffset(), cellBO.getCABO().getStride(), model.context.FLOAT, 3, model.context.FALSE)) vtkErrorMacro("Error setting vertexMC in shader VAO.");
			}
			if (cellBO.getProgram().isAttributeUsed("tcoordMC") && cellBO.getCABO().getTCoordOffset()) {
				if (!cellBO.getVAO().addAttributeArray(cellBO.getProgram(), cellBO.getCABO(), "tcoordMC", cellBO.getCABO().getTCoordOffset(), cellBO.getCABO().getStride(), model.context.FLOAT, cellBO.getCABO().getTCoordComponents(), model.context.FALSE)) vtkErrorMacro("Error setting tcoordMC in shader VAO.");
			}
		...
		const iComps = actor.getProperty().getIndependentComponents();
		if (iComps) for (let i = 0; i < numComp; i++) cellBO.getProgram().setUniformf(`mix${i}`, actor.getProperty().getComponentWeight(i));
		...
		for (let i = 0; i < numComp; i++) {
			...
			if (cfun && actor.getProperty().getUseLookupTableScalarRange()) {
				...
				cl = .5 * (cRange[1] + cRange[0]);
			}
			const scale = oglShiftScale.scale / cw;
			const shift = (oglShiftScale.shift - cl) / cw + .5;
		...
		for (let i = 0; i < numComp; i++) {
			...
			if (pwfun) {
				...
				const mid = .5 * (pwfRange[0] + pwfRange[1]);
				pwfScale = oglShiftScale.scale / length;
				pwfShift = (oglShiftScale.shift - mid) / length + .5;
			}
	...
	publicAPI.setCameraShaderParameters = (cellBO, ren, actor) => {
		...
		if (actor.getProperty().getUseLabelOutline() === true) {
			...
			if (sliceAxis === SlicingMode.NONE) sliceAxis = SlicingMode.K;
	...
	publicAPI.setPropertyShaderParameters = (cellBO, ren, actor) => {
		const program = cellBO.getProgram();
		const opacity = actor.getProperty().getOpacity();
	...
	publicAPI.buildBufferObjects = (ren, actor) => {
		...
		for (let component = 0; component < numIComps; ++component) colorTransferFunctions.push(actorProperty.getRGBTransferFunction(component));
		...
		if (!cTex?.oglObject?.getHandle() || cTex?.hash !== cfunToString) {
			...
			if (firstColorTransferFunc) {
				...
				for (let c = 0; c < numIComps; c++) {
					...
					else for (let i = 0; i < cWidth * 3; i++) cTable[c * cWidth * 6 + i] = 255 * tmpTable[i];
				}
		...
		for (let component = 0; component < numIComps; ++component) opacityFunctions.push(actorProperty.getPiecewiseFunction(component));
		...
		if (!pwfTex?.oglObject?.getHandle() || pwfTex?.hash !== pwfunToString) {
			...
			model.pwfTexture = Texture_default.newInstance({ resizable: true });
			...
			if (firstPwFunc) {
				...
				for (let c = 0; c < numIComps; ++c) {
					...
					if (pwfun === null) pwfFloatTable.fill(1);
						...
						else for (let i = 0; i < pwfWidth; i++) pwfFloatTable[c * pwfWidth * 2 + i] = tmpTable[i];
					}
				}
		...
		if (ijkMode !== model.renderable.getSlicingMode()) slice = model.renderable.getSliceAtPosition(slice);
		...
		let sliceOffset;
		if (ijkMode === SlicingMode.I) sliceOffset = nSlice - ext[0];
		if (ijkMode === SlicingMode.J) sliceOffset = nSlice - ext[2];
		if (ijkMode === SlicingMode.K || ijkMode === SlicingMode.NONE) sliceOffset = nSlice - ext[4];
		...
		if (model.VBOBuildString !== toString) {
			...
			if (!model.openGLTexture) model.openGLTexture = Texture_default.newInstance({ resizable: true });
			...
			if (iType === InterpolationType.NEAREST) {
				if (new Set([
					...
					3,
					4
				]).has(numComp) && dataType === VtkDataTypes.UNSIGNED_CHAR && !iComps) {
					model.openGLTexture.setGenerateMipmap(true);
					model.openGLTexture.setMinificationFilter(Filter.NEAREST);
				} else model.openGLTexture.setMinificationFilter(Filter.NEAREST);
				...
				} else model.openGLTexture.setMinificationFilter(Filter.LINEAR);
			...
			const sliceDepth = [
				...
				SlicingMode.Y,
				SlicingMode.Z
			].includes(model.renderable.getSlicingMode()) ? slice : nSlice;
			...
			if (ijkMode === SlicingMode.I) {
				...
				let id = 0;
				for (let k = 0; k < dims[2]; k++) for (let j = 0; j < dims[1]; j++) {
					let bsIdx = (sliceOffset + j * dims[0] + k * dims[0] * dims[1]) * numComp;
					id = (k * dims[1] + j) * numComp;
					const end = bsIdx + numComp;
					while (bsIdx < end) scalars[id++] = basicScalars[bsIdx++];
				}
				dims[0] = dims[1];
				dims[1] = dims[2];
				...
				let id = 0;
				for (let k = 0; k < dims[2]; k++) for (let i = 0; i < dims[0]; i++) {
			...
			} else vtkErrorMacro("Reformat slicing not yet supported.");
			/**
			*
	...
	publicAPI.updateLabelOutlineOpacityTexture = (image) => {
		...
		if (typeof labelOutlineOpacity === "number") {
			if (model._cachedLabelOutlineOpacityObj?.[0] === labelOutlineOpacity) labelOutlineOpacity = model._cachedLabelOutlineOpacityObj;
			else labelOutlineOpacity = [labelOutlineOpacity];
		...
		if (!lTex?.oglObject?.getHandle() || lTex?.hash !== toString) {
			...
			for (let i = 0; i < lWidth; ++i) lTable[i] = labelOutlineOpacity[i] ?? labelOutlineOpacity[0];
			model.labelOutlineOpacityTexture = Texture_default.newInstance({ resizable: false });
		...
		} else model.labelOutlineOpacityTexture = lTex.oglObject;
	};
	publicAPI.updatelabelOutlineThicknessTexture = (image) => {
		...
		if (!lTex?.oglObject?.getHandle() || lTex?.hash !== toString) {
			...
			model.labelOutlineThicknessTexture = Texture_default.newInstance({ resizable: false });
...
var ImageMapper_default = {
	newInstance,
	extend
};
registerOverride("vtkAbstractImageMapper", newInstance);
//#endregion
export { ImageMapper_default as default, extend, newInstance };

//# sourceMappingURL=ImageMapper.js.map