...
import ViewNodeFactory_default, { registerOverride } from "./ViewNodeFactory.js";
import RenderPass_default from "../SceneGraph/RenderPass.js";
import ForwardPass_default from "./ForwardPass.js";
import HardwareSelector_default from "./HardwareSelector.js";
import ShaderCache_default from "./ShaderCache.js";
import TextureUnitManager_default from "./TextureUnitManager.js";
import RenderWindowViewNode_default from "../SceneGraph/RenderWindowViewNode.js";
import { createContextProxyHandler } from "./RenderWindow/ContextProxy.js";
//#region Sources/Rendering/OpenGL/RenderWindow/index.js
...
function vtkOpenGLRenderWindow(publicAPI, model) {
	...
	function getCachingContextHandler() {
		if (!cachingContextHandler) cachingContextHandler = createContextProxyHandler();
	...
	function updateWindow() {
		...
		if (model.manageCanvas) model.canvas.style.display = model.useOffScreen ? "none" : "block";
	...
	publicAPI.initialize = () => {
		if (!model.initialized) {
			...
			if (model.rootOpenGLRenderWindow) model.context2D = publicAPI.get2DContext();
			else {
				model.context = publicAPI.get3DContext();
				publicAPI.resizeFromChildRenderWindows();
				if (model.context) createGLContext();
				model.textureUnitManager = TextureUnitManager_default.newInstance();
	...
	publicAPI.setContainer = (el) => {
		if (model.el && model.el !== el) {
			if (model.canvas.parentNode !== model.el) vtkErrorMacro("Error: canvas parent node does not match container");
	...
	publicAPI.getFramebufferSize = () => {
		return model.activeFramebuffer?.getSize() || model.size;
	};
	...
	publicAPI.get3DContext = (options = {
		...
		const result = typeof WebGL2RenderingContext !== "undefined" ? model.canvas.getContext("webgl2", options) : null;
		if (result) vtkDebugMacro("using WebGL2");
		if (!result) vtkErrorMacro("no WebGL2 context");
		return new Proxy(result, getCachingContextHandler());
	};
	publicAPI.get2DContext = (options = {}) => model.canvas.getContext("2d", options);
	publicAPI.restoreContext = () => {
		const rp = RenderPass_default.newInstance();
	...
	publicAPI.getDefaultTextureByteSize = (vtkType, oglNorm16Ext = null, useHalfFloat = false) => {
		switch (vtkType) {
			...
			case VtkDataTypes.UNSIGNED_CHAR: return 1;
			case oglNorm16Ext:
			case useHalfFloat:
			case VtkDataTypes.UNSIGNED_SHORT:
			case VtkDataTypes.SHORT:
			case VtkDataTypes.VOID: return 2;
			default: return 4;
		}
	};
	publicAPI.getDefaultTextureInternalFormat = (vtktype, numComps, oglNorm16Ext = null, useHalfFloat = false) => {
		switch (vtktype) {
			case VtkDataTypes.UNSIGNED_CHAR: switch (numComps) {
				case 1: return model.context.R8;
				case 2: return model.context.RG8;
				case 3: return model.context.RGB8;
				default: return model.context.RGBA8;
			}
			case oglNorm16Ext && !useHalfFloat && VtkDataTypes.UNSIGNED_SHORT: switch (numComps) {
				case 1: return oglNorm16Ext.R16_EXT;
				case 2: return oglNorm16Ext.RG16_EXT;
				case 3: return oglNorm16Ext.RGB16_EXT;
				default: return oglNorm16Ext.RGBA16_EXT;
			}
			case oglNorm16Ext && !useHalfFloat && VtkDataTypes.SHORT: switch (numComps) {
				case 1: return oglNorm16Ext.R16_SNORM_EXT;
				case 2: return oglNorm16Ext.RG16_SNORM_EXT;
				case 3: return oglNorm16Ext.RGB16_SNORM_EXT;
				default: return oglNorm16Ext.RGBA16_SNORM_EXT;
			}
			case VtkDataTypes.UNSIGNED_SHORT:
			case VtkDataTypes.SHORT:
			case VtkDataTypes.FLOAT:
			default: switch (numComps) {
				case 1: return useHalfFloat ? model.context.R16F : model.context.R32F;
				case 2: return useHalfFloat ? model.context.RG16F : model.context.RG32F;
				case 3: return useHalfFloat ? model.context.RGB16F : model.context.RGB32F;
				default: return useHalfFloat ? model.context.RGBA16F : model.context.RGBA32F;
			}
		}
	};
	...
	publicAPI.setUseBackgroundImage = (value) => {
		...
		if (model.useBackgroundImage && !model.el.contains(model.bgImage)) model.el.appendChild(model.bgImage);
		else if (!model.useBackgroundImage && model.el.contains(model.bgImage)) model.el.removeChild(model.bgImage);
	};
	function getCanvasDataURL(format = model.imageFormat) {
		...
		model.renderable.getRenderers().forEach((renderer) => {
			renderer.getViewProps().forEach((viewProp) => {
				if (viewProp.getContainer) {
					const canvasList = viewProp.getContainer().getElementsByTagName("canvas");
	...
	publicAPI.captureNextImage = (format = "image/png", { resetCamera = false, size = null, scale = 1 } = {}) => {
		if (model.deleted) return null;
		let screenshotSize = !!size || scale !== 1 ? size || model.size.map((val) => val * scale) : null;
		if (screenshotSize !== null && !model.manageCanvas) {
			vtkWarningMacro("Ignoring the requested screenshot size/scale: resizing the canvas requires manageCanvas=true on vtkOpenGLRenderWindow. Capturing at the current size instead.");
			screenshotSize = null;
		}
		model.imageFormat = format;
		const previous = model.notifyStartCaptureImage;
		model.notifyStartCaptureImage = true;
		model._screenshot = { size: screenshotSize };
				...
				if (model._screenshot.size === null) {
					...
					if (resetCamera) {
						...
						model._screenshot.cameras = model.renderable.getRenderers().map((renderer) => {
							...
							return {
								resetCameraArgs: isUserResetCamera ? { renderer } : void 0,
	...
	publicAPI.getHardwareMaximumLineWidth = () => {
		if (hardwareMaximumLineWidth != null) return hardwareMaximumLineWidth;
	...
	publicAPI.getGLInformations = () => {
		if (model._glInformation) return model._glInformation;
		const gl = publicAPI.get3DContext();
		...
		const params = [
			...
			[
				...
				[
					checkRenderTargetSupport(gl, gl.RGBA8, gl.RGBA, gl.UNSIGNED_BYTE) ? "RGBA8" : "",
					checkRenderTargetSupport(gl, gl.RGB8, gl.RGB, gl.UNSIGNED_BYTE) ? "RGB8" : "",
					checkRenderTargetSupport(gl, gl.RG8, gl.RG, gl.UNSIGNED_BYTE) ? "RG8" : "",
					checkRenderTargetSupport(gl, gl.R8, gl.RED, gl.UNSIGNED_BYTE) ? "R8" : ""
				].join(" ")
			],
			[
				...
				[
					glColorBufferFloat && checkRenderTargetSupport(gl, gl.RGBA16F, gl.RGBA, gl.HALF_FLOAT) ? "RGBA16F" : "",
					glColorBufferFloat && checkRenderTargetSupport(gl, gl.RG16F, gl.RG, gl.HALF_FLOAT) ? "RG16F" : "",
					glColorBufferFloat && checkRenderTargetSupport(gl, gl.R16F, gl.RED, gl.HALF_FLOAT) ? "R16F" : ""
				].join(" ")
			],
			[
				...
				[
					glColorBufferFloat && checkRenderTargetSupport(gl, gl.RGBA32F, gl.RGBA, gl.FLOAT) ? "RGBA32F" : "",
					glColorBufferFloat && checkRenderTargetSupport(gl, gl.RG32F, gl.RG, gl.FLOAT) ? "RG32F" : "",
					glColorBufferFloat && checkRenderTargetSupport(gl, gl.R32F, gl.RED, gl.FLOAT) ? "R32F" : ""
				].join(" ")
			],
			[
	...
	publicAPI.traverseAllPasses = () => {
		if (model.renderPasses) for (let index = 0; index < model.renderPasses.length; ++index) model.renderPasses[index].traverse(publicAPI, null);
		publicAPI.copyParentContent();
		if (model.notifyStartCaptureImage) getCanvasDataURL();
	...
	publicAPI.copyParentContent = () => {
		...
		if (!rootParent || !model.context2D || model.children.some((oglRenderer) => !!oglRenderer.getSelector?.())) return;
	...
	publicAPI.setViewStream = (stream) => {
		if (model.viewStream === stream) return false;
	...
	publicAPI.createSelector = () => {
		const ret = HardwareSelector_default.newInstance();
	...
	publicAPI.delete = macros_default.chain(() => {
		if (model.context) deleteGLContext();
	...
	publicAPI.setSize = (...args) => {
		...
		if (modified) publicAPI.invokeWindowResizeEvent({
			width: model.size[0],
			height: model.size[1]
		});
		return modified;
	...
	publicAPI.registerGraphicsResourceUser = (coreObject, newUser) => {
		if (!model._graphicsResources.has(coreObject)) publicAPI.setGraphicsResourceForObject(coreObject, null, null);
		model._graphicsResources.get(coreObject)?.users.add(newUser);
	};
	...
	publicAPI.releaseGraphicsResources = () => {
		if (model.shaderCache !== null) model.shaderCache.releaseGraphicsResources(publicAPI);
		model._graphicsResources.forEach(({ oglObject }) => {
			oglObject.releaseGraphicsResources(publicAPI);
		});
		model._graphicsResources.clear();
		if (model.textureUnitManager !== null) model.textureUnitManager.freeAll();
		model.renderable.getRenderersByReference().forEach((ren) => {
			publicAPI.getViewNodeFor(ren)?.releaseGraphicsResources();
		});
	};
	const publicAPIBeforeProxy = { ...publicAPI };
	parentMethodsToProxy.forEach((methodName) => {
		publicAPI[methodName] = (...args) => {
			if (model.rootOpenGLRenderWindow) return model.rootOpenGLRenderWindow[methodName](...args);
...
function extend(publicAPI, model, initialValues = {}) {
	...
	RenderWindowViewNode_default.extend(publicAPI, model, initialValues);
	if (!model.canvas) {
		model.canvas = document.createElement("canvas");
		model.canvas.style.width = "100%";
	}
	if (!model.selector) {
		model.selector = HardwareSelector_default.newInstance();
	...
	model.myFactory = ViewNodeFactory_default.newInstance();
	model.shaderCache = ShaderCache_default.newInstance();
	model.shaderCache.setOpenGLRenderWindow(publicAPI);
	model.renderPasses[0] = ForwardPass_default.newInstance();
	...
	macros_default.setGetArray(publicAPI, model, ["size"], 2);
	macros_default.event(publicAPI, model, "imageReady");
	macros_default.event(publicAPI, model, "windowResizeEvent");
	vtkOpenGLRenderWindow(publicAPI, model);
}
var newInstance = macros_default.newInstance(extend, "vtkOpenGLRenderWindow");
...
//#endregion
export { RenderWindow_default as default, extend, newInstance, popMonitorGLContextCount, pushMonitorGLContextCount };
