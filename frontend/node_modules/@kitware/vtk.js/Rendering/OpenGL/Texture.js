import { get, macros_exports, moveToProtected, newInstance as newInstance$1, newTypedArray, obj, set, setGet } from "../../macros.js";
import { nearestPowerOfTwo } from "../../Common/Core/Math.js";
import DataArray_default from "../../Common/Core/DataArray.js";
import HalfFloat_default from "../../Common/Core/HalfFloat.js";
import Constants_default from "./Texture/Constants.js";
...
//#region Sources/Rendering/OpenGL/Texture/index.js
var { Wrap, Filter } = Constants_default;
var { VtkDataTypes } = DataArray_default;
var { vtkDebugMacro, vtkErrorMacro, vtkWarningMacro, requiredParam } = macros_exports;
var { toHalf } = HalfFloat_default;
function vtkOpenGLTexture(publicAPI, model) {
	...
	publicAPI.render = (renWin = null) => {
		if (renWin) model._openGLRenderWindow = renWin;
		else {
		...
		if (model.renderable.getInterpolate()) {
			if (model.generateMipmap) publicAPI.setMinificationFilter(Filter.LINEAR_MIPMAP_LINEAR);
			else publicAPI.setMinificationFilter(Filter.LINEAR);
		...
		if (model.renderable.getInputData()) model.renderable.setImage(null);
		if (!model.handle || model.renderable.getMTime() > model.textureBuildTime.getMTime()) {
			...
			if (input && input.getPointData().getScalars()) {
				...
				for (let i = 0; i < model.renderable.getNumberOfInputPorts(); ++i) {
					...
					if (scalars) data.push(scalars);
				}
				if (model.renderable.getInterpolate() && inScalars.getNumberOfComponents() === 4) {
					model.generateMipmap = true;
					publicAPI.setMinificationFilter(Filter.LINEAR_MIPMAP_LINEAR);
				}
				if (data.length % 6 === 0) publicAPI.createCubeFromRaw({
					width: ext[1] - ext[0] + 1,
				...
				else publicAPI.create2DFromRaw({
					width: ext[1] - ext[0] + 1,
					height: ext[3] - ext[2] + 1,
					numComps: inScalars.getNumberOfComponents(),
					dataType: inScalars.getDataType(),
		...
		if (model.handle) publicAPI.activate();
	};
	const getNorm16Ext = () => {
		if ((model.minificationFilter === Filter.LINEAR || model.magnificationFilter === Filter.LINEAR) && !supportsNorm16LinearCached()) return;
		return model.oglNorm16Ext;
	};
	publicAPI.destroyTexture = () => {
		publicAPI.deactivate();
		if (model.context && model.handle) model.context.deleteTexture(model.handle);
	...
	publicAPI.getTextureUnit = () => {
		if (model._openGLRenderWindow) return model._openGLRenderWindow.getTextureUnitForTexture(publicAPI);
		return -1;
	};
	publicAPI.activate = () => {
	...
	publicAPI.bind = () => {
		model.context.bindTexture(model.target, model.handle);
		if (model.autoParameters && publicAPI.getMTime() > model.sendParametersTime.getMTime()) publicAPI.sendParameters();
	};
	publicAPI.isBound = () => {
		let result = false;
		if (model.context && model.handle) {
			...
			result = model.context.getIntegerv(target) === model.handle;
		}
		return result;
	};
	publicAPI.sendParameters = () => {
	...
	publicAPI.getInternalFormat = (vtktype, numComps) => {
		if (!model._forceInternalFormat) model.internalFormat = publicAPI.getDefaultInternalFormat(vtktype, numComps);
		...
		].includes(model.internalFormat) && !model.context.getExtension("OES_texture_float_linear")) vtkWarningMacro("Failed to load OES_texture_float_linear. Texture filtering is not available for *32F internal formats.");
	...
	publicAPI.getDefaultFormat = (vtktype, numComps) => {
		switch (numComps) {
			case 1: return model.context.RED;
			case 2: return model.context.RG;
			case 3: return model.context.RGB;
			case 4: return model.context.RGBA;
			default: return model.context.RGB;
		}
	};
	publicAPI.resetFormatAndType = () => {
		model._prevTexParams = null;
	...
	publicAPI.getDefaultDataType = (vtkScalarType) => {
		...
		switch (vtkScalarType) {
			case VtkDataTypes.UNSIGNED_CHAR: return model.context.UNSIGNED_BYTE;
			case getNorm16Ext() && !useHalfFloat && VtkDataTypes.SHORT: return model.context.SHORT;
			case getNorm16Ext() && !useHalfFloat && VtkDataTypes.UNSIGNED_SHORT: return model.context.UNSIGNED_SHORT;
			case useHalfFloat && VtkDataTypes.SHORT:
			case useHalfFloat && VtkDataTypes.UNSIGNED_SHORT: return model.context.HALF_FLOAT;
			case VtkDataTypes.FLOAT:
			case VtkDataTypes.VOID:
			default: return model.context.FLOAT;
		}
	};
	publicAPI.getOpenGLDataType = (vtkScalarType, forceUpdate = false) => {
		if (!model.openGLDataType || forceUpdate) model.openGLDataType = publicAPI.getDefaultDataType(vtkScalarType);
	...
	publicAPI.getOpenGLFilterMode = (emode) => {
		switch (emode) {
			case Filter.NEAREST: return model.context.NEAREST;
			case Filter.LINEAR: return model.context.LINEAR;
			case Filter.NEAREST_MIPMAP_NEAREST: return model.context.NEAREST_MIPMAP_NEAREST;
			case Filter.NEAREST_MIPMAP_LINEAR: return model.context.NEAREST_MIPMAP_LINEAR;
			case Filter.LINEAR_MIPMAP_NEAREST: return model.context.LINEAR_MIPMAP_NEAREST;
			case Filter.LINEAR_MIPMAP_LINEAR: return model.context.LINEAR_MIPMAP_LINEAR;
			default: return model.context.NEAREST;
		}
	};
	publicAPI.getOpenGLWrapMode = (vtktype) => {
		switch (vtktype) {
			case Wrap.CLAMP_TO_EDGE: return model.context.CLAMP_TO_EDGE;
			case Wrap.REPEAT: return model.context.REPEAT;
			case Wrap.MIRRORED_REPEAT: return model.context.MIRRORED_REPEAT;
			default: return model.context.CLAMP_TO_EDGE;
		}
	};
	/**
	...
	function readExtentIntoArray(data, dataDims, extent, outArray, outOffset) {
		...
		for (let zi = zmin; zi <= zmax; zi++) {
			...
			for (let yi = ymin; yi <= ymax; yi++) {
				...
				for (let readOffset = zyOffset + xmin, end = zyOffset + xmax; readOffset <= end; readOffset++, writeOffset++) outArray[writeOffset] = data[readOffset];
			}
		}
	}
	/**
	...
	*/
	function readExtents(data, extents, typedArrayConstructor = null) {
		const extentPixels = new (typedArrayConstructor || data.constructor)(extents.reduce((count, extent) => count + getExtentPixelCount(extent), 0));
		const dataDims = [
			model.width,
			model.height,
			model.depth
	...
	*/
	publicAPI.updateArrayDataTypeForGL = (dataType, data, depth = false, imageExtents = []) => {
		...
		if (depth) pixCount *= model.depth;
		const onlyUpdateExtents = !!imageExtents.length;
		if (dataType !== VtkDataTypes.FLOAT && model.openGLDataType === model.context.FLOAT) for (let idx = 0; idx < data.length; idx++) if (data[idx]) if (onlyUpdateExtents) pixData.push(readExtents(data[idx], imageExtents, Float32Array));
		else {
			const dataArrayToCopy = data[idx].length > pixCount ? data[idx].subarray(0, pixCount) : data[idx];
			pixData.push(new Float32Array(dataArrayToCopy));
		}
		else pixData.push(null);
		if (dataType !== VtkDataTypes.UNSIGNED_CHAR && model.openGLDataType === model.context.UNSIGNED_BYTE) for (let idx = 0; idx < data.length; idx++) if (data[idx]) if (onlyUpdateExtents) pixData.push(readExtents(data[idx], imageExtents, Uint8Array));
		else {
			const dataArrayToCopy = data[idx].length > pixCount ? data[idx].subarray(0, pixCount) : data[idx];
			pixData.push(new Uint8Array(dataArrayToCopy));
		}
		else pixData.push(null);
		if (model.openGLDataType === model.context.HALF_FLOAT) for (let idx = 0; idx < data.length; idx++) if (data[idx]) {
			...
			for (let i = 0; i < newArrayLen; i++) newArray[i] = toHalf(src[i]);
			pixData.push(newArray);
		} else pixData.push(null);
		if (pixData.length === 0) for (let i = 0; i < data.length; i++) pixData.push(onlyUpdateExtents && data[i] ? readExtents(data[i], imageExtents) : data[i]);
		return pixData;
	};
	function useTexStorage(dataType) {
		if (model._openGLRenderWindow) {
			if (model.resizable || model.renderable?.getResizable()) return false;
			if (model._openGLRenderWindow.getGLInformations().RENDERER.value.match(/WebKit/gi) && navigator.platform.match(/Mac/gi) && getNorm16Ext() && (dataType === VtkDataTypes.UNSIGNED_SHORT || dataType === VtkDataTypes.SHORT)) return false;
			return true;
		}
		return false;
	}
	publicAPI.create2DFromRaw = ({ width = requiredParam("width"), height = requiredParam("height"), numComps = requiredParam("numComps"), dataType = requiredParam("dataType"), data = requiredParam("data"), flip = false } = {}) => {
		...
		if (useTexStorage(dataType)) {
			...
			if (pixData[0] != null) model.context.texSubImage2D(model.target, 0, 0, 0, model.width, model.height, model.format, model.openGLDataType, pixData[0]);
		} else model.context.texImage2D(model.target, 0, model.internalFormat, model.width, model.height, 0, model.format, model.openGLDataType, pixData[0]);
	...
	publicAPI.createCubeFromRaw = ({ width = requiredParam("width"), height = requiredParam("height"), numComps = requiredParam("numComps"), dataType = requiredParam("dataType"), data = requiredParam("data") } = {}) => {
		...
		for (let i = 0; i < pixData.length; i++) {
			...
			for (let y = 0; y < heightLevel; ++y) {
				...
				invertedData[i].set(pixData[i].slice(row2, row2 + widthLevel * model.components), row1);
			}
		}
		model.context.pixelStorei(model.context.UNPACK_ALIGNMENT, 1);
		if (useTexStorage(dataType)) model.context.texStorage2D(model.target, 6, model.internalFormat, model.width, model.height);
		for (let i = 0; i < 6; i++) {
			let j = 0;
			let w = model.width;
			let h = model.height;
			while (w >= 1 && h >= 1) {
				let tempData = null;
				if (j <= model.maxLevel) tempData = invertedData[6 * j + i];
				if (useTexStorage(dataType)) {
					if (tempData != null) model.context.texSubImage2D(model.context.TEXTURE_CUBE_MAP_POSITIVE_X + i, j, 0, 0, w, h, model.format, model.openGLDataType, tempData);
				} else model.context.texImage2D(model.context.TEXTURE_CUBE_MAP_POSITIVE_X + i, j, model.internalFormat, w, h, 0, model.format, model.openGLDataType, tempData);
				j++;
				w /= 2;
				h /= 2;
			}
	...
	publicAPI.createDepthFromRaw = ({ width = requiredParam("width"), height = requiredParam("height"), dataType = requiredParam("dataType"), data = requiredParam("data") } = {}) => {
		publicAPI.getOpenGLDataType(dataType);
		model.format = model.context.DEPTH_COMPONENT;
		if (dataType === VtkDataTypes.FLOAT) model.internalFormat = model.context.DEPTH_COMPONENT32F;
		else model.internalFormat = model.context.DEPTH_COMPONENT16;
		if (!model.internalFormat || !model.format || !model.openGLDataType) {
			vtkErrorMacro("Failed to determine texture parameters.");
			return false;
		}
		...
		if (useTexStorage(dataType)) {
			model.context.texStorage2D(model.target, 1, model.internalFormat, model.width, model.height);
			if (data != null) model.context.texSubImage2D(model.target, 0, 0, 0, model.width, model.height, model.format, model.openGLDataType, data);
		} else model.context.texImage2D(model.target, 0, model.internalFormat, model.width, model.height, 0, model.format, model.openGLDataType, data);
		if (model.generateMipmap) model.context.generateMipmap(model.target);
	...
	publicAPI.create2DFromImage = (image) => {
		...
		} else model.context.texImage2D(model.target, 0, model.internalFormat, model.width, model.height, 0, model.format, model.openGLDataType, textureSource);
		if (model.generateMipmap) model.context.generateMipmap(model.target);
	...
	publicAPI.create2DFromImageBitmap = (imageBitmap) => {
		...
		} else model.context.texImage2D(model.target, 0, model.internalFormat, model.width, model.height, 0, model.format, model.openGLDataType, imageBitmap);
		if (model.generateMipmap) model.context.generateMipmap(model.target);
		model.allocatedGPUMemoryInBytes = model.width * model.height * model.depth * model.components * model._openGLRenderWindow.getDefaultTextureByteSize(VtkDataTypes.UNSIGNED_CHAR, getNorm16Ext(), publicAPI.useHalfFloat());
		publicAPI.deactivate();
		return true;
	...
	function hasExactHalfFloat(offset, scale) {
		for (let c = 0; c < offset.length; c++) {
			...
			if (min < -2048 || min > 2048 || max < -2048 || max > 2048) return false;
		}
		return true;
	}
	function setCanUseHalfFloat(dataType, offset, scale, preferSizeOverAccuracy) {
		...
		model.canUseHalfFloat = (model.openGLDataType === model.context.FLOAT && model.context.getExtension("OES_texture_float_linear") === null && isExactHalfFloat || model.openGLDataType === model.context.HALF_FLOAT) && isExactHalfFloat;
	}
	...
	publicAPI.create2DFilterableFromRaw = ({ width = requiredParam("width"), height = requiredParam("height"), numComps = requiredParam("numComps"), dataType = requiredParam("dataType"), data = requiredParam("data"), preferSizeOverAccuracy = false, ranges = void 0 } = {}) => publicAPI.create2DFilterableFromDataArray({
		width,
		height,
		dataArray: DataArray_default.newInstance({
	...
	publicAPI.create2DFilterableFromDataArray = ({ width = requiredParam("width"), height = requiredParam("height"), dataArray = requiredParam("dataArray"), preferSizeOverAccuracy = false } = {}) => {
		const { numComps, dataType, data } = processDataArray(dataArray, preferSizeOverAccuracy);
		publicAPI.create2DFromRaw({
			width,
			height,
			numComps,
	...
	publicAPI.updateVolumeInfoForGL = (dataType, numComps) => {
		...
		const useHalfFloat = publicAPI.useHalfFloat();
		if (!model.volumeInfo?.scale || !model.volumeInfo?.offset) model.volumeInfo = {
		...
		if (getNorm16Ext() && !useHalfFloat && dataType === VtkDataTypes.SHORT) {
			for (let c = 0; c < numComps; ++c) model.volumeInfo.scale[c] = 32767;
			isScalingApplied = true;
		}
		if (getNorm16Ext() && !useHalfFloat && dataType === VtkDataTypes.UNSIGNED_SHORT) {
			for (let c = 0; c < numComps; ++c) model.volumeInfo.scale[c] = 65535;
			isScalingApplied = true;
		}
		if (dataType === VtkDataTypes.UNSIGNED_CHAR) {
			for (let c = 0; c < numComps; ++c) model.volumeInfo.scale[c] = 255;
			isScalingApplied = true;
		}
	...
	publicAPI.create3DFromRaw = ({ width = requiredParam("width"), height = requiredParam("height"), depth = requiredParam("depth"), numComps = requiredParam("numComps"), dataType = requiredParam("dataType"), data = requiredParam("data"), updatedExtents = [] } = {}) => {
		...
		if (!publicAPI.updateVolumeInfoForGL(dataTypeToUse, numComps) && dataToUse) {
			...
			for (let i = 0; i < numPixelsIn; i++) for (let nc = 0; nc < numComps; nc++) {
		...
		const pixData = publicAPI.updateArrayDataTypeForGL(dataTypeToUse, dataArray, true, rebuildEntireTexture ? [] : updatedExtents);
		model.context.pixelStorei(model.context.UNPACK_ALIGNMENT, 1);
		if (rebuildEntireTexture) {
			if (useTexStorage(dataTypeToUse)) {
				...
				if (pixData[0] != null) model.context.texSubImage3D(model.target, 0, 0, 0, 0, model.width, model.height, model.depth, model.format, model.openGLDataType, pixData[0]);
			} else model.context.texImage3D(model.target, 0, model.internalFormat, model.width, model.height, model.depth, 0, model.format, model.openGLDataType, pixData[0]);
	...
	publicAPI.create3DFilterableFromRaw = ({ width = requiredParam("width"), height = requiredParam("height"), depth = requiredParam("depth"), numComps = requiredParam("numComps"), dataType = requiredParam("dataType"), data = requiredParam("data"), preferSizeOverAccuracy = false, ranges = void 0, updatedExtents = [] } = {}) => publicAPI.create3DFilterableFromDataArray({
		width,
		height,
		depth,
		dataArray: DataArray_default.newInstance({
	...
	publicAPI.create3DFilterableFromDataArray = ({ width = requiredParam("width"), height = requiredParam("height"), depth = requiredParam("depth"), dataArray = requiredParam("dataArray"), preferSizeOverAccuracy = false, updatedExtents = [] } = {}) => {
		const { numComps, dataType, data, scaleOffsets } = processDataArray(dataArray, preferSizeOverAccuracy);
		const offset = [];
		const scale = [];
		for (let c = 0; c < numComps; ++c) {
			offset[c] = 0;
	...
	publicAPI.getMaximumTextureSize = (ctx) => {
		if (ctx && ctx.isCurrent()) return ctx.getIntegerv(ctx.MAX_TEXTURE_SIZE);
		return -1;
	};
...
function extend(publicAPI, model, initialValues = {}) {
	...
	obj(model.sendParametersTime, { mtime: 0 });
	model.textureBuildTime = {};
	obj(model.textureBuildTime, { mtime: 0 });
...
//#endregion
export { Texture_default as default, extend, newInstance };
